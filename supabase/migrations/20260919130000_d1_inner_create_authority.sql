-- D1 v4 correction round 2 (M24): bỏ veto cấp course trên inner create-exercise path.
--
-- `d1_create_exercise_with_content_unchecked` vẫn gate bằng `has_course_management_access`
-- (course-scoped, chỉ owner/co_owner/editor) trong khi `create_exercise_with_content` đã chuyển
-- sang `d1_prepare_topic_content_mutation` → `d1_topic_group_member` từ 20260916140000. Hệ quả:
-- creator bị hạ course role xuống `previewer` qua được gate ngoài rồi bị chặn ở gate trong
-- với COURSE_EDIT_FORBIDDEN. Đây là drift, không phải chủ ý: sibling
-- `d1_soft_delete_exercise_cascade_unchecked` đã được sửa sang helper topic-scoped từ trước.
--
-- Chỉ thay đúng một điều kiện authorization. Giữ nguyên tên, signature, SECURITY DEFINER,
-- search_path, AUTH_REQUIRED và toàn bộ validation phía dưới, cùng bề mặt EXECUTE hiện có
-- (`*_unchecked` không được grant cho `authenticated`; chỉ wrapper gọi nó).

create or replace function public.d1_create_exercise_with_content_unchecked(
  p_topic_id uuid,
  p_payload jsonb
)
returns jsonb
...
begin
  ...
  -- M24: cùng topic-content authority với edit/delete/card mutation, không giữ veto cấp course.
  if not public.can_modify_content_by_topic(p_topic_id) then
