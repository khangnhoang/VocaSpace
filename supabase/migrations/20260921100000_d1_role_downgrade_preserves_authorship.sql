-- Hạ vai trò chỉ thay membership và quyền duyệt; authorship của topic là một
-- quan hệ độc lập, chỉ được chuyển khi thành viên bị gỡ hoặc chủ động rời course.
...
begin
  ...
  if not public.is_course_owner_or_co_owner(v_target.course_id) then
  ...
  if p_role <> v_target.role then
    ...
    where i.course_id = v_target.course_id
      and i.role = p_role
  ...
  v_new_flag := case
    ...
    when v_target.role = 'previewer'::public.course_member_role
      and p_role = 'editor'::public.course_member_role then v_target.can_review_topics
    else false
  end;

  perform public.d1_assert_pending_review_reviewer_safety(
    v_target.id,
    p_role,
    v_new_flag,
    false
  ...
  set role = p_role,
      can_review_topics = v_new_flag
  where id = v_target.id;

  return jsonb_build_object(
    'status', 'updated',
    'collaborator_id', v_target.id,
    'role', p_role,
    'can_review_topics', v_new_flag
  );
end;
...
revoke all on function public.update_course_collaborator_role(uuid, public.course_member_role)
  from public, anon, authenticated;
grant execute on function public.update_course_collaborator_role(uuid, public.course_member_role)
  to authenticated, service_role;

drop function if exists public.update_course_collaborator_role_with_responsibility(
  uuid,
