...
function mapTopicAuthorshipError(error?: { message?: string }) {
  ...
  if (text.includes("TOPIC_AUTHORSHIP_MANAGEMENT_FORBIDDEN")) return "Chỉ owner hoặc co-owner mới được quản lý nhóm tác giả topic.";
  if (text.includes("TOPIC_CONTRIBUTOR_MEMBERSHIP_REQUIRED")) return "Cộng tác viên phải là thành viên có quyền soạn nội dung trong khóa học.";
  if (text.includes("TOPIC_RESPONSIBLE_AUTHOR_NOT_CONTRIBUTOR")) return "Responsible author không thể đồng thời là contributor.";
  if (text.includes("TOPIC_CONTRIBUTOR_CAP_REACHED")) return "Topic đã đạt tối đa 2 contributor.";
  if (text.includes("TOPIC_RESPONSIBILITY_RECIPIENT_INVALID")) return "Người nhận trách nhiệm không hợp lệ cho topic này.";
  if (text.includes("TOPIC_PENDING_FROZEN")) return "Topic đang chờ duyệt và tạm thời không nhận thay đổi.";
  if (text.includes("TOPIC_NOT_FOUND") || text.includes("TOPIC_REMOVED")) return "Topic không còn khả dụng.";
  return "Không thể cập nhật nhóm tác giả topic. Vui lòng thử lại.";
}
...
export async function addTopicContributor(rawInput: TopicContributorInput) {
  ...
  if (!parsed.success) return { error: parsed.error.issues[0]?.message ?? "Dữ liệu contributor không hợp lệ." };
...
export async function removeTopicContributor(rawInput: TopicContributorRemovalInput) {
  ...
  if (!parsed.success) return { error: parsed.error.issues[0]?.message ?? "ID contributor không hợp lệ." };
  const supabase = await getAuthenticatedClient();
  if (!supabase) return { error: "Vui lòng đăng nhập lại." };
...
export async function transferTopicResponsibility(rawInput: TopicResponsibilityTransferInput) {
  ...
  if (!parsed.success) return { error: parsed.error.issues[0]?.message ?? "Dữ liệu transfer không hợp lệ." };
  const supabase = await getAuthenticatedClient();
  if (!supabase) return { error: "Vui lòng đăng nhập lại." };
