...
- Use `.agents/skills/playwright-cli/SKILL.md` when a task requires automating browser interactions, testing or verifying a web page through a browser, or authoring, running, debugging, or healing Playwright tests. The presence of `@playwright/test`, `playwright.config.*`, existing smoke E2E coverage, an ambiently open browser, or a source-only discussion does not activate this skill by itself.
- Playwright CLI owns browser-driving mechanics for matching repository tasks. `.agents/skills/test-quality-strategy/SKILL.md` still owns whether browser evidence is required, the appropriate test layer, fixture readiness, verification scope, and evidence claims, while the frontend skills own UI acceptance. When another generic browser-driver skill overlaps, use Playwright CLI for the browser-driving commands unless the owner explicitly requests another driver or Playwright cannot access the required surface; do not run multiple drivers for the same check without a concrete need.
