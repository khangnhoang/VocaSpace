...
7. Self-review the candidate for exact Owner fidelity, repository ownership, bidirectional traceability, lifecycle liveness/boundedness, implementability, verification truthfulness, and unnecessary mechanisms. For every delegated semantic, confirm that the delegated canonical owner actually contains the required contract, its acceptance evidence targets behavior at that owner rather than only the delegation sentence, and the plan remains usable without a phase-specific prompt supplying reusable contract semantics. The candidate remains planning-only.
