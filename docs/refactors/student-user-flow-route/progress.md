...
| Q7 — Internal previewer access correction | P0–P3 đã commit cục bộ (`66561ba`, `1becd9f`, `0278389`, `f39c49b`); independent review phát hiện hai Required và correction đã kiểm tra local, pre-deploy media inventory môi trường đích chưa thực hiện. | Sau D1 và trước D2; sửa previewer-only read/media boundary cùng enrollment gate cho Action và direct Data API, giữ DB-wide relation/correctness invariants khác trong `LEARNING-INTEGRITY-001`. |
