...
| PR D1: Topic authoring → review → publication (`FUTURE-PUBLISH-001`) | Earlier local implementation P0–P4 và fresh-reader corrections đã đạt; senior-review correction đã tái xác minh, đang self-review/commit local | C2 PR #96 đã merge | `feat/topic-publish-validation`; local-only implementation; checkpoint commits tiếp tục theo amendment | 2026-09-17 | Historical P0–P4 evidence vẫn giữ cho contract trước amendment. Senior correction đã đóng 5 findings bằng migration/Server Action/DTO/UI/regression evidence; final local cleanup reset và handoff report còn lại. Browser/fixture/service-role và repository-wide lint residual vẫn được ghi rõ. |
...
- Trạng thái: Detailed plan đã được amend; P0 foundation, P1 trusted lifecycle boundary, P2 content mutation safety, P3 workflow UX và P4 closure đã đạt local checkpoint **theo contract trước amendment** trên `feat/topic-publish-validation`; correction units sau fresh-reader đã commit local `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `43c8fcd`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df`, `0b27b85`, `98879ec`, `6a6f577`; senior-review correction đã verify và self-review PASS, final cleanup reset local đã hoàn tất. **Amendment thứ hai (2026-09-18) bỏ escalation/rescue/review-budget nay ĐÃ implement xong P1–P5** — code/migration/test trong repo phản ánh contract sau amendment (xem mục "D1 correction v2 — implementation closure" bên dưới); **chưa commit**, chưa push.
- Detailed plan: contract hiện hành là [implementation-plans/d1/correction-plan-deepseek-v2.md](./implementation-plans/d1/correction-plan-deepseek-v2.md) (Owner pass 2026-09-18; đã implement P1–P5). [implementation-plans/d1/plan.md](./implementation-plans/d1/plan.md) **giữ stale có chủ đích** — vẫn mô tả contract trước amendment. Điều kiện reconcile theo **D14** ("sau khi v2 implement xong (P1–P5)") **nay đã thoả**, nên việc reconcile `plan.md` **đang đến hạn và chưa thực hiện**. Owner request hiện tại là decision source, không có owner-review brief riêng.
