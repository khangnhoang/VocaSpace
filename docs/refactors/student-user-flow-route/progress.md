...
| Wave D: Later backlog | D1 đã merge; Q7 đã rollout production; D2 trở đi chưa triển khai | Stable route/dashboard/workspace contracts | D1 PR #98; Q7 PR #101, merge `65e8481`; working execution order trong [plan.md](./plan.md) | 2026-09-24 | Q7 hoàn tất gate rollout trong phạm vi đã xác minh; D2 là unit tiếp theo. D6–D9 deferred/open về detailed acceptance. |
...
| D2 — Preview topic contract | `C1–C3` đã triển khai, kiểm tra và commit cục bộ; `C4–C6` chưa hoàn tất. Marker/quota/lifecycle và UI quản lý Preview cho giáo viên đã có; public preview read access và trải nghiệm Preview chưa mở. | [D2 detail plan](./implementation-plans/d2/plan.md) và [thiết kế](./implementation-plans/d2/design.md) sở hữu contract; mỗi checkpoint có evidence và commit cục bộ riêng. |
