...
| PR D1: Topic authoring → review → publication (`FUTURE-PUBLISH-001`) | Earlier local implementation P0–P4 và fresh-reader corrections đã đạt; amended P0-A/P1-A/P2-A/P3-A đã PASS local, P4-A còn mở | C2 PR #96 đã merge | `feat/topic-publish-validation`; local-only implementation; checkpoint commits tiếp tục theo amendment | 2026-09-16 | Historical P0–P4 evidence vẫn giữ cho contract trước amendment. D1 tiếp tục `P4-A` cho final invariant/browser/manual closure; P3-A authorship workflow/feedback/collaborator UI và create-result navigation đã có local evidence. Browser/fixture/service-role và repository-wide lint residual vẫn được ghi rõ. |
...
- Trạng thái: Detailed plan đã được amend; P0 foundation, P1 trusted lifecycle boundary, P2 content mutation safety, P3 workflow UX và P4 closure đã đạt local checkpoint **theo contract trước amendment** trên `feat/topic-publish-validation`; correction units sau fresh-reader đã commit local `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `43c8fcd`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df`, `0b27b85`; amended P0-A/P1-A/P2-A đã PASS local, P3-A/P4-A còn mở; chưa push.
...
- Amended checkpoint P0-A/P1-A/P2-A/P3-A (2026-09-16): P0-A thêm topic authorship/provenance foundation; P1-A chuyển topic group, responsibility transfer/leave, reviewer exclusion và membership transitions qua trusted boundary; P2-A áp dụng topic-group authorization cho topic/card/exercise/question-group/question/option/restore/media, đóng legacy RPC arity bypass, re-check group sau course/topic lock cho direct Data API trigger và harden `move_topic_order`; P3-A thêm workflow read DTO/UI cho authorship group, persistent transfer feedback, collaborator summary/leave warnings, responsibility-safe role/removal paths và interaction regression cho create→builder với `0/1/multiple` draft hiện có. P0-A/P1-A/P2-A đã có checkpoint commit; P3-A đã PASS local và chờ checkpoint commit trong closure hiện tại. Focused UI/action/schema `5 files / 63 tests`, affected integration `12 files / 127 tests`, full unit `55 files / 463 tests`, TypeScript và targeted ESLint đạt; full lint còn 7 lỗi baseline ngoài phạm vi. P4-A còn mở cho final invariant/browser/manual closure.
...
| --- | --- | --- |
| D1 — Topic authoring → review → publication | Earlier P0–P4 đã có readiness/review foundation, trusted lifecycle boundary, content mutation safety, workflow UX và closure local; amended P0-A/P1-A/P2-A đã PASS local trên branch này; P3-A navigation/UX và P4-A closure còn mở. | Tiếp tục tuần tự `P3-A → P4-A`; chưa handoff review/push/PR cho amended contract; không mở rộng sang Q7/D2/candidate revision. |
