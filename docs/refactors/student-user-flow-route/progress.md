...
| Q7 — Internal previewer access correction | Đang triển khai: P0/P1 đã commit cục bộ; P2 enrollment-write local checkpoint đạt sau self-review, P3 consumer/manual QA chưa hoàn tất. | Sau D1 và trước D2; sửa previewer-only read/media boundary cùng enrollment gate cho Action và direct Data API, giữ DB-wide relation/correctness invariants khác trong `LEARNING-INTEGRITY-001`. |
