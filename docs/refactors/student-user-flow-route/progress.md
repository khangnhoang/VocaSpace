...
| PR D1: Topic authoring → review → publication (`FUTURE-PUBLISH-001`) | Local implementation P0–P4 đã đạt; chưa push | C2 PR #96 đã merge | `feat/topic-publish-validation`; local-only implementation | 2026-09-15 | P0 foundation, P1 trusted lifecycle boundary, P2 content mutation safety, P3 workflow UX và P4 closure đã đạt local. D1 vẫn sở hữu readiness, review lifecycle, pending freeze, reviewer capability, separate admin moderation boundary và interim published-edit demotion; browser matrix đầy đủ và repository-wide lint còn residual đã ghi rõ. |
...
- Trạng thái: Detailed plan đã hoàn tất; P0 foundation, P1 trusted lifecycle boundary, P2 content mutation safety, P3 workflow UX và P4 closure đã đạt local checkpoint trên `feat/topic-publish-validation`; chưa push.
...
- Escalation contract: rejection counter scope `topic + submitted_by`; sau reject lần thứ ba topic về `draft`, escalation unresolved, submitter bị giữ request review topic đó và tạo topic mới trong course; không global account lock. Owner/co_owner rescue hoặc close/abandon có audit để giải phóng hold.
...
- Trạng thái: Implementation-ready planning complete; audit toàn bộ backlog và Owner decision reconciliation đã hoàn tất ngày 2026-09-15. D1 P0–P4 đã đạt local; các residual và giới hạn claim được ghi ở checkpoint P4.
...
- Đã triển khai: `getCourseCollaboratorOverview` đọc membership/capability qua action có schema DTO; owner/co_owner có thể mở bounded capability/role/removal controls hiện có, còn role downgrade/removal/last-reviewer safety vẫn nằm ở trusted RPC. Không mở rộng invite flow. Admin page hiện vẫn là mock/unimplemented nên không thêm live moderation UI giả; P1 moderation boundary vẫn là contract/runtime surface.
- Verification: local reset với migration tới `20260915120000_d1_topic_workflow_read.sql` đạt; topic-review và course-readiness integration `29/29`; focused action/schema/component/unit `9 files / 139 tests`; TypeScript `npx.cmd tsc --noEmit --incremental false` đạt; targeted ESLint `0 errors`, 2 warning pre-existing trong `AddExerciseDialog.tsx`; `git diff --check` đạt, chỉ có line-ending warnings.
...
- Kết quả: PASS local sau self-review và correction; chưa push.
- Correction: sau mutation flashcard/exercise thành công nhưng không có dashboard issue context, `TopicBuilderTabs` gọi `router.refresh()` để revalidate readiness/lifecycle DTO của parent Server Component. Manual QA trước correction đã chứng minh workflow panel giữ sai `0/0`; sau correction panel chuyển đúng `1/0`, rồi `1/1`.
- Browser/manual evidence: canonical teacher route đã chạy bounded flow create topic → builder draft `0/0` với CTA disabled → thêm flashcard → readiness `1/0` → thêm exercise → readiness `1/1` và CTA enabled → request review → `pending` freeze với các mutation controls disabled. Full cross-role D1 browser matrix chưa được claim vì fixture hiện tại chưa đủ; existing smoke suite còn `7 passed / 5 failed` do các test cũ dùng `/courses/...` và assumptions builder cũ, không phải canonical `/teacher/courses/...` D1 flow.
- Verification sau correction: full Vitest `50 files / 441 tests` passed; D1 topic-review + course-readiness integration `2 files / 29 tests` passed; TypeScript `npx.cmd tsc --noEmit --incremental false` passed; production build passed sau rerun ngoài sandbox để tải Google Fonts; targeted ESLint cho D1 closure `0 errors` với `2 warnings` pre-existing ở `AddExerciseDialog.tsx`; `git diff --check` không có lỗi nội dung.
- Repository-wide lint vẫn không xanh: `7 errors / 13 warnings` ở baseline ngoài D1 diff, theo dõi tại `QUALITY-001`; không dùng kết quả này để che hoặc hạ chuẩn D1 targeted lint.
- Self-review: đã áp dụng `docs/agent-self-review.md` sau correction; rà lại readiness refresh, pending freeze, published confirmation/demotion, review-vs-moderation boundary, previewer read-only, collaborator capability, direct-write hardening, service-role residual và stale smoke evidence. Không còn finding material mở ảnh hưởng D1 acceptance trong local evidence.
- Residual không phải Owner blocker: 11 active `published` seed fixture topics thiếu active exercise chưa được remediation vì không có production-data authority; `service_role` vẫn là infrastructure bypass; media upload còn staging artifact; admin moderation boundary có runtime/RPC/audit nhưng current admin page vẫn mock; full accessibility/responsive và cross-role browser QA chưa hoàn tất; stale smoke tests cần follow-up riêng.
- Closure: P0→P4 đã có local checkpoint commit riêng; không có Owner-decision blocker. Handoff chỉ sẵn sàng cho bước review/push/PR riêng, không claim merge readiness cho các browser/fixture residual chưa verified.
