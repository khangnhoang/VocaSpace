...
| Wave D: Later backlog | D1 đã merge; Q7 trở đi chưa triển khai theo tracker hiện tại | Stable route/dashboard/workspace contracts | D1 PR #98; working execution order trong [plan.md](./plan.md) | 2026-09-23 | D1 là Wave D implementation đầu tiên đã merge; order D1 → Q7 → D2 → D3 → D4 → D5 → D6 → D7 → D8 → D9; D6–D9 deferred/open về detailed acceptance. |
...
| Q7 — Internal previewer access correction | P0–P2 đã commit cục bộ; P3 consumer/manual QA và self-review đạt local checkpoint, chờ local commit. Pre-deploy media inventory môi trường đích chưa thực hiện. | Sau D1 và trước D2; sửa previewer-only read/media boundary cùng enrollment gate cho Action và direct Data API, giữ DB-wide relation/correctness invariants khác trong `LEARNING-INTEGRITY-001`. |
...
- D1 goal-level decisions và canonical detail plan đã được triển khai, merge qua PR #98; Owner đã cấp implementation và local-checkpoint commit authority riêng cho Q7, không cấp push/PR/deploy/remote DB authority. D2 vẫn cần unit contract và authority riêng. D3–D5 còn semantic decisions riêng, còn D6–D9 giữ deferred/open về detailed acceptance và sẽ quyết định khi làm tới.
...
- Self-review theo `docs/agent-self-review.md` đạt: truy đường access check → graph read → route CTA, partial Structure stats, Builder D1 capability và persisted media GET theo actor; kiểm tra diff không có thay đổi ngoài P3, `git diff --check` đạt, không còn finding material mở. Local commit P3 đang chờ; pre-deploy managed-media inventory/remediation ở môi trường đích vẫn là stop gate riêng, chưa được cấp authority. Local Q7 pass không đồng nghĩa rollout/deploy pass.
