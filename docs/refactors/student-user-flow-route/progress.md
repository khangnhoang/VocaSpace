...
| PR D1: Topic authoring → review → publication (`FUTURE-PUBLISH-001`) | Earlier local implementation P0–P4 và fresh-reader corrections đã đạt; **amended topic-authorship/navigation contract chưa implement** | C2 PR #96 đã merge | `feat/topic-publish-validation`; local-only implementation; latest correction chain `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df`, `0b27b85` | 2026-09-16 | Historical P0–P4 evidence vẫn giữ cho contract trước amendment. D1 cần reopen `P0-A → P1-A → P2-A → P3-A → P4-A` cho immutable topic creator/responsible author/contributors, initial-review exclusion, atomic transfer/self-leave, group-scoped content writes, transfer feedback và create-result navigation regression; browser/fixture/service-role và repository-wide lint residual vẫn được ghi rõ. |
...
| --- | --- | --- |
| D1 — Topic authoring → review → publication | Earlier P0–P4 đã có readiness/review foundation, trusted lifecycle boundary, content mutation safety, workflow UX và closure local; amended topic authorship/responsibility/navigation chưa implement; browser matrix đầy đủ/stale baseline còn residual. | Reopen `P0-A → P1-A → P2-A → P3-A → P4-A`; chưa handoff review/push/PR cho amended contract; không mở rộng sang Q7/D2/candidate revision. |
