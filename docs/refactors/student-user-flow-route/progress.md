...
| D2 — Preview topic contract | `C1–C4` đã triển khai, kiểm tra và commit cục bộ; `C5–C6` chưa hoàn tất. Marker/quota/lifecycle, quản lý Preview cho giáo viên và guarded public service đã có; trải nghiệm Preview chưa mở. | [D2 detail plan](./implementation-plans/d2/plan.md) và [thiết kế](./implementation-plans/d2/design.md) sở hữu contract; mỗi checkpoint có evidence và commit cục bộ riêng. |
