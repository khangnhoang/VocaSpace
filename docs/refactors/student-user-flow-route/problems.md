...
- Trạng thái vấn đề: Open; detailed plan đã hoàn tất, chưa có implementation.
...
- P0 inventory finding: seeded DB hiện có 11 active `published` fixture topics thiếu active exercise hoặc card (tất cả thiếu active exercise). P0 không tự hạ status hoặc sửa dữ liệu; trước khi P1 bật DB invariant phải có migration/fixture remediation contract và exact preflight evidence cho các row này.
- Open after P0: lifecycle transition/RLS hardening, pending freeze, separate moderation boundary, published mutation atomicity và concurrency guarantees vẫn chưa được triển khai; đây là P1/P2 scope, không phải P0 blocker.
