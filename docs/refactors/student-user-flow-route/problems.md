...
- Trạng thái vấn đề: Open; detailed plan đã được amend, D1 P0–P4 và correction units sau fresh-reader đã triển khai trên branch local **theo contract trước amendment**, correction chain hiện gồm `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `43c8fcd`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df`, `0b27b85`. Full unit `52 files / 453 tests` đạt; full integration chạy trong sandbox đạt `119/121`, hai test harness gọi `supabase db query` bị lỗi quyền ghi telemetry, còn focused D1 suites đạt. Các giới hạn fixture/browser và service-role guarantee vẫn là risk/follow-up; amended authorship contract chưa implement nên không được claim handoff READY.
...
- Ảnh hưởng còn mở: Nếu P2-A bỏ sót một content/card/exercise/child/media path, outside-group collaborator vẫn có thể edit hoặc published/pending semantics bị lệch; P3-A còn phải làm discoverable transfer feedback và kiểm chứng create navigation. Các invariant schema/transfer/reviewer exclusion của P0-A/P1-A đã có local evidence.
- Hướng xử lý: P0-A đã thêm minimal authorship schema/history/fixture backfill policy; P1-A đã đóng trusted group/transfer/leave/request boundary, reviewer exclusion, reviewer-safety và locking. Tiếp tục P2-A cho content-write authorization; P3-A cho UI/feedback/navigation; P4-A cho invariant/concurrency/browser evidence. Không dùng reviewer delegation boolean để thay thế authorship relation.

### D1-NAV-001: Create topic navigation theo nhiều draft chưa có evidence interaction

- Trạng thái vấn đề: Open discovery; root cause runtime chưa xác nhận.
- Evidence: `create_topic_ordered` dùng `INSERT ... RETURNING *`; `createTopic` trả `result.topic`; `TopicManagementSheet` lấy `res.data.id` và push `getTopicBuilderPath(courseId, createdTopicId)`. Không tìm thấy `topics[0]`, current-draft fallback hoặc selection từ reload. Existing route suite mock `TopicManagementSheet`, action test chỉ kiểm tra RPC, và chưa có 0/1/multiple draft interaction coverage.
...
- Hướng xử lý: P3-A phải tái hiện bounded trên 0/1/multiple draft, assert exact returned topic id và route arrival, rồi mới sửa đúng boundary nếu có reproduction. Không mở rộng thành UI redesign.
