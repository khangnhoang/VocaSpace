...
- Open after correction: D1 local implementation đã qua P0→P4 và correction; amended P0-A/P1-A/P2-A/P3-A/P4-A đã pass local theo contract hiện tại. Existing smoke suite còn `7 passed / 5 failed` vì dùng stale `/courses/...` routes và builder assumptions; không dùng làm product evidence và không modernize trong D1. Full cross-role/accessibility/responsive browser matrix chưa được claim vì fixture chưa đủ. Admin moderation surface hiện vẫn là mock nên không được claim là live UI evidence. Question-group media upload vẫn là content-media staging path ngoài thumbnail correction; ordinary Storage DELETE lifecycle boundary đã đóng, nhưng published media replace/delete transaction semantics vẫn chưa có supported application flow và không được invent trong D1. Service-role/seed writes vẫn là giới hạn database-wide guarantee đã ghi nhận; 11 invalid published seed fixtures chưa remediation vì không có production-data authority. Đây là follow-up/implementation debt, không phải Owner-decision blocker.
