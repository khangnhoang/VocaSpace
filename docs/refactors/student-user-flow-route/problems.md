...
- Trạng thái vấn đề: Open; detailed plan đã được amend, D1 P0–P4 và correction units sau fresh-reader đã triển khai trên branch local **theo contract trước amendment**, correction chain hiện gồm `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `43c8fcd`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df`, `0b27b85`. Full unit `55 files / 463 tests` đạt; affected amended integration `12 files / 127 tests` đạt; repository-wide lint còn 7 lỗi baseline ngoài phạm vi. P0-A/P1-A/P2-A/P3-A đã pass local; các giới hạn fixture/browser và service-role guarantee vẫn là risk/follow-up; amended P4-A còn mở nên chưa claim handoff READY.
...
- Open after correction: D1 local implementation đã qua P0→P4 và correction **chỉ theo contract trước amendment**; amended P0-A/P1-A/P2-A đã pass local, còn P3-A navigation/UX và P4-A closure nên handoff amended hiện chưa READY. Existing smoke suite còn `7 passed / 5 failed` vì dùng stale `/courses/...` routes và builder assumptions; full cross-role/accessibility/responsive browser matrix chưa được claim vì fixture chưa đủ. Admin moderation surface hiện vẫn là mock nên không được claim là live UI evidence. Question-group media upload vẫn là content-media staging path ngoài thumbnail correction; failed Storage cleanup về bản chất vẫn là best-effort vì Storage và DB không cùng transaction. Service-role/seed writes vẫn là giới hạn database-wide guarantee đã ghi nhận; 11 invalid published seed fixtures chưa remediation vì không có production-data authority. Đây là follow-up/implementation debt của amendment; không phải Owner-decision blocker nếu schema/history/feedback shape vẫn được chốt trong engineering gates.

### D1-AUTHORSHIP-001: Topic-level authorship/responsibility chưa được mô hình hóa

- Trạng thái vấn đề: Đã xử lý ở P0-A/P1-A/P2-A/P3-A; P4-A còn giữ closure evidence.
- Mô tả: Baseline trước amended checkpoints chỉ có course membership role và `can_review_topics`; P0-A đã thêm immutable topic creator/responsible, contributor group và pre-first-approval evidence; P1-A đã chuyển `get_topic_workflow_state`, `request_topic_review`, transfer/self-leave và reviewer exclusion qua trusted boundary. P2-A vẫn phải áp dụng topic-group predicate cho toàn bộ content mutation closure và P3-A phải hoàn tất feedback/navigation UX.
- Ảnh hưởng còn mở: P2-A đã đóng content/card/exercise/child/media group boundary và direct-trigger TOCTOU; P3-A đã làm discoverable transfer feedback và kiểm chứng create navigation bằng interaction regression. Các invariant schema/transfer/reviewer exclusion của P0-A/P1-A, content boundary của P2-A và UI/read boundary của P3-A đã có local evidence; P4-A vẫn phải hoàn tất closure/browser evidence.
- Hướng xử lý: P0-A đã thêm minimal authorship schema/history/fixture backfill policy; P1-A đã đóng trusted group/transfer/leave/request boundary, reviewer exclusion, reviewer-safety và locking; P2-A đã đóng content-write authorization, legacy RPC bypass và direct-trigger TOCTOU; P3-A đã thêm workflow read/UI, collaborator summary/leave warnings, persistent transfer feedback và authoritative create navigation evidence. P4-A còn sở hữu invariant/concurrency/browser closure. Không dùng reviewer delegation boolean để thay thế authorship relation.

### D1-NAV-001: Create topic navigation theo nhiều draft chưa có evidence interaction

- Trạng thái vấn đề: Đã xử lý bằng interaction evidence; không có source root-cause correction vì runtime premise không tái hiện.
- Evidence: `create_topic_ordered` dùng `INSERT ... RETURNING *`; `createTopic` trả `result.topic`; `TopicManagementSheet` lấy `res.data.id` và push `getTopicBuilderPath(courseId, createdTopicId)`. P3-A component interaction test chạy với `0/1/multiple` existing drafts và luôn assert exact newly-created id; full unit và TypeScript đạt. Không tìm thấy `topics[0]`, current-draft fallback hoặc selection từ reload.
- Rủi ro: Có thể còn cạnh tranh giữa `router.push`, `refreshTopics`, parent revalidation hoặc dashboard `router.replace`, nhưng đây mới là inference cần reproduction; không ghi nhận “chọn nhầm draft” là confirmed root cause.
- Hướng xử lý: Giữ source hiện tại và regression test authoritative returned id; không mở rộng thành UI redesign. P4-A chỉ cần đưa evidence này vào final closure và phân biệt rõ với browser runtime reproduction nếu môi trường chưa cung cấp.
