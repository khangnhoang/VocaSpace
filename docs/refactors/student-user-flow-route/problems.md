...
- Open after correction: D1 local implementation đã qua P0→P4 và correction **chỉ theo contract trước amendment**; amended topic-authorship/navigation contract chưa implement nên handoff hiện không READY. Existing smoke suite còn `7 passed / 5 failed` vì dùng stale `/courses/...` routes và builder assumptions; full cross-role/accessibility/responsive browser matrix chưa được claim vì fixture chưa đủ. Admin moderation surface hiện vẫn là mock nên không được claim là live UI evidence. Question-group media upload vẫn là content-media staging path ngoài thumbnail correction; failed Storage cleanup về bản chất vẫn là best-effort vì Storage và DB không cùng transaction. Service-role/seed writes vẫn là giới hạn database-wide guarantee đã ghi nhận; 11 invalid published seed fixtures chưa remediation vì không có production-data authority. Đây là follow-up/implementation debt của amendment; không phải Owner-decision blocker nếu schema/history/feedback shape vẫn được chốt trong engineering gates.

### D1-AUTHORSHIP-001: Topic-level authorship/responsibility chưa được mô hình hóa

- Trạng thái vấn đề: Open; Owner contract đã chốt, implementation chưa bắt đầu.
- Mô tả: Current D1 chỉ có course membership role và `can_review_topics`; không có immutable topic creator, current responsible author, explicit contributor group, pre-first-approval responsibility evidence, transfer/self-leave boundary hoặc feedback surface. `get_topic_workflow_state` và `request_topic_review` hiện cho course-authoring member edit/request theo role/readiness, nên outside-group authoring collaborator và contributor/responsible distinction chưa được enforce.
- Ảnh hưởng: Có thể giao edit hoặc submit/resubmit sai người; remove/role mutation có thể làm topic thiếu responsible author; chỉ dựa vào current membership sẽ không enforce được initial-review exclusion sau transfer/remove; contributor cap và feedback chưa có trusted source of truth.
- Hướng xử lý: Reopen `P0-A` cho minimal authorship schema/history/fixture backfill policy; `P1-A` cho trusted group/transfer/leave/request boundary và locking; `P2-A` cho content-write authorization; `P3-A` cho UI/feedback; `P4-A` cho invariant/concurrency/browser evidence. Không dùng reviewer delegation boolean để thay thế authorship relation.
