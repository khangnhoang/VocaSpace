...
- Trạng thái vấn đề: Open; detailed plan đã hoàn tất, D1 P0–P4 và correction units sau fresh-reader đã triển khai trên branch local, correction chain hiện gồm `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `43c8fcd`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df`, `0b27b85`. Full unit `52 files / 453 tests` đạt; full integration chạy trong sandbox đạt `119/121`, hai test harness gọi `supabase db query` bị lỗi quyền ghi telemetry, còn focused D1 suites đạt. Các giới hạn fixture/browser và service-role guarantee vẫn là risk/follow-up, không phải Owner-decision blocker.
...
- Open after correction: D1 local implementation đã qua P0→P4 và correction; existing smoke suite còn `7 passed / 5 failed` vì dùng stale `/courses/...` routes và builder assumptions; full cross-role/accessibility/responsive browser matrix chưa được claim vì fixture chưa đủ. Admin moderation surface hiện vẫn là mock nên không được claim là live UI evidence. Question-group media upload vẫn là content-media staging path ngoài thumbnail correction; failed Storage cleanup về bản chất vẫn là best-effort vì Storage và DB không cùng transaction. Service-role/seed writes vẫn là giới hạn database-wide guarantee đã ghi nhận; 11 invalid published seed fixtures chưa remediation vì không có production-data authority. Đây là follow-up/merge-evidence debt, không phải Owner-decision blocker cho implementation boundary hiện tại.
