...
| --- | --- | --- | --- |
| D1 — Topic authoring → review → publication | **Outcome state:** Đã triển khai local trên `feat/topic-publish-validation`; P0–P4 và các correction unit đã đạt, chưa push/PR. Current implementation dùng trusted RPC/RLS/locking cho readiness, review, pending freeze, moderation, profile self-delete guard và collaborator role-capacity; residual browser/fixture/service-role limits nằm trong [progress.md](./progress.md) và [problems.md](./problems.md). | D1 sở hữu basic new-topic authoring→review→publication workflow, readiness hai loại active content, pending freeze, reviewer/effective permission, no-self-review, rejection và interim published-edit demotion. DB/RPC/RLS/direct Data API, soft-delete/restore, moderation separation và concurrency đã được audit/đóng trong implementation boundary. | Standalone cross-layer D1 implementation local; next action là fresh-reader rereview rồi mới xem xét push/PR riêng. Future candidate revisions chỉ là compatibility constraint. |
