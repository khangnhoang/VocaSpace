...
| Q7 — Internal previewer access correction | **Outcome state:** P0–P3 đã triển khai, kiểm tra và commit cục bộ trên `docs/student-user-flow-q7-plan`; independent review phát hiện correction đã kiểm tra local; pre-deploy inventory môi trường đích chưa thực hiện, chưa push/rollout. | Course active có thể draft/pending/published, nhưng previewer-only chỉ đọc topic active/published kể cả media; giữ capability D1 độc lập. Ghi persistent progress/answer/review qua Action và Data API cần enrollment vào course của target; các relation/correctness invariants khác vẫn thuộc `LEARNING-INTEGRITY-001`. Đây là prerequisite authorization cho D2, không phải full invite/ownership rollout. | Bounded collaborator/access correction sau D1, trước D2; [Q7 detail plan](./implementation-plans/q7/plan.md) sở hữu lookup, policy, media và verification gates. |
...
10. Q7: Internal previewer access correction — local implementation sau D1 đã có; pre-deploy gate vẫn chặn rollout trước D2; formal PR title/brief sẽ chốt trong unit riêng.
...
Mỗi Wave D candidate chưa triển khai cần implementation brief và Owner acceptance riêng trước khi code; D1 đã merge qua PR #98, không mở lại gate này theo trạng thái cũ. [Canonical D1 detail plan](./implementation-plans/d1/plan.md) sở hữu contract đã triển khai; Q7/D2 vẫn cần authority theo unit riêng, còn D6 trở đi giữ deferred/open cho đến khi làm tới và có acceptance tương ứng. Quyết định Owner lịch sử của D1 không tự cấp quyền cho follow-up mới.
