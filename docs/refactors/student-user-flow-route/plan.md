...
- Reviewer là course-scoped capability: `owner`/`co_owner` có implicit permission; `editor`/`previewer` chỉ review khi `course_collaborators.can_review_topics = true`; mutation phải kiểm tra effective permission hiện tại và không được làm pending submission mất reviewer hợp lệ cuối cùng. Global `admin` role alone không cấp review authority.
- Global `admin` vẫn giữ platform moderation/maintenance authority tách biệt: trusted demotion/takedown hoặc pending-review invalidation có mandatory moderation audit, không tăng rejection counter và không được direct-publish/bypass readiness.
