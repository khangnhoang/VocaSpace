...
- **D1 topic authorship amendment (2026-09-16):** course authoring eligibility không tự biến user thành topic author. Mỗi topic có immutable original creator, đúng một responsible author và tối đa hai explicit contributors; responsible author duy nhất request/resubmit, contributors chỉ edit, ngoài group read-only. Trước first successful approval, original creator/current responsible/initial contributors/pre-approval responsible users đều bị loại khỏi reviewer; transfer hoặc self-leave phải xử lý atomic trước membership mutation, chỉ nhận owner/co_owner hoặc existing contributor hợp lệ, và transfer feedback phải có topic/course context. Create navigation luôn dùng authoritative newly-created topic id, độc lập số lượng draft hiện có; implementation follow-up phải reopen D1 checkpoints.
...
| --- | --- | --- | --- |
| D1 — Topic authoring → review → publication | **Outcome state:** Earlier P0–P4 and fresh-reader corrections are implemented locally on `feat/topic-publish-validation`, but the 2026-09-16 topic-authorship/navigation amendment is not implemented. Current evidence remains valid only for the earlier contract; amended follow-up must reopen `P0-A → P1-A → P2-A → P3-A → P4-A`. | D1 now also owns immutable topic creator provenance, one responsible author, max two contributors, group-scoped edit/submit, initial-review exclusion, atomic transfer/self-leave and authoritative create-result navigation. Full visual redesign, email/push delivery and candidate revisions remain deferred. | Standalone cross-layer follow-up on the current branch; do not treat the prior handoff as implementation-ready for the amended contract. Future candidate revisions remain compatibility constraint. |
...
9. PR D1: Topic authoring → review → publication — implementation local đã đạt sau C2; chờ fresh-reader rereview trước push/PR. Singular-owner invariant và collaborator role-control polish đã được verify trong correction hiện tại.
