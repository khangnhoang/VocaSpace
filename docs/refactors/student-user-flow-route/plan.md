...
| Q7 — Internal previewer access correction | **Outcome state:** Chưa triển khai; `has_course_content_read_access` hiện cho collaborator đọc content quá rộng, chưa lọc topic `published`; learner-write Action và RLS chưa yêu cầu enrollment. | Course active có thể draft/pending/published, nhưng previewer-only chỉ đọc topic active/published kể cả media; giữ capability D1 độc lập. Ghi persistent progress/answer/review qua Action và Data API cần enrollment vào course của target; các relation/correctness invariants khác vẫn thuộc `LEARNING-INTEGRITY-001`. Đây là prerequisite authorization cho D2, không phải full invite/ownership rollout. | Bounded collaborator/access correction sau D1, trước D2; [Q7 detail plan](./implementation-plans/q7/plan.md) sở hữu lookup, policy, media và verification gates. |
