...
- Lần rà soát program/dependency và ownership gần nhất: 2026-09-25 (reconcile D2 local delivery; D3 là candidate kế tiếp).
...
12. Preview topic (D2) đã hoàn tất và qua independent review trên nhánh local: owner/co-owner/editor được quản lý marker theo [D2 detail plan](./implementation-plans/d2/plan.md); quota là `ceil(active_topic_count * 20%)`, trong đó topic active cần cả topic và chapter cha chưa bị xóa, draft vẫn tính vào denominator nhưng chưa public. Public Preview chỉ mở cho course published/public và topic published + active + preview marker.
...
| `/teacher/courses/[id]` | Course overview/dashboard | Auth active collaborator; course-local capability theo membership | Readiness/next action dashboard | Quản lý cấu trúc hoặc sửa issue theo capability | Wave A/D1 |
| `/teacher/courses/[id]/structure` | Course structure workspace | Auth active collaborator; `owner|co_owner|editor` quản lý cấu trúc, topic content/delete theo capability riêng | Chapters/topics | Add/edit/soft-delete/reorder theo từng capability | Wave A/D1 |
| `/teacher/courses/[id]/topics/[topicId]` | Topic builder | Auth active collaborator; content edit theo topic authoring group, kể cả previewer thuộc group | Flashcards/exercises/settings | Lưu nội dung topic theo topic capability; ngoài group read-only | Wave A/D1 |
...
- Course-local authority derive từ active membership và topic-level authorship, không chỉ từ role: `owner`/`co_owner`/`editor` quản lý cấu trúc theo capability; original creator, responsible author và contributor được sửa content của topic mình khi còn membership, kể cả khi role hiện tại là `previewer`. Collaborator ngoài authoring group không sửa content topic đó. `student` và `admin` vẫn có thể giữ collaborator membership và nhận đúng local authority; global role không suppress local authoring.
...
- Course creation qua normal boundary tạo đúng một active `owner`; invitation và normal collaborator role mutation không tạo `owner` thứ hai, `co_owner` là role đặc quyền bổ sung, và DB partial unique guard enforce tối đa một owner. Ownership transfer vẫn ngoài D1.
...
- Đây là program contract; [canonical D1 detail plan](./implementation-plans/d1/plan.md) sở hữu schema/RPC/trigger/locking, UI boundary và acceptance chi tiết đã triển khai. Quyết định và quyền thực hiện từng bước trong lịch sử D1 không tự cấp quyền cho thay đổi tiếp theo, deploy hoặc remote DB mutation.
...
- **Q7 internal previewer:** course active có thể ở `draft`, `pending` hoặc `published`; previewer-only chỉ đọc topic active `published` gồm media, còn topic-scoped D1 authoring/review capability độc lập được giữ. Persistent progress/answer/review write qua Action và Data API đòi enrollment thật vào course của target topic/question/card; collaborator role không cấp quyền đó. Q7 không ôm các DB-wide relation/correctness invariant khác của `LEARNING-INTEGRITY-001`.
- **D2 public preview:** course active và `published/public`; topic và chapter cha active, topic `published` và có preview marker. Guest, learner chưa enroll hoặc learner đã enroll nhưng chủ động vào Preview được xem toàn bộ content của topic, gồm media; có thể trả lời và nhận đúng/sai tức thời, nhưng không lưu progress, answer, review hoặc learning state nào từ Preview.
- **D2 quota lifecycle:** active topic cần cả topic và chapter cha chưa bị xóa; draft tính vào denominator nhưng marker trên draft chưa public. Giữ hard cap sau ordinary mutation, với ngoại lệ moderation có audit và đường phục hồi đã chốt ở [D2 detail plan](./implementation-plans/d2/plan.md). Owner/co_owner/editor được quản lý marker; xóa topic hoặc chapter tự gỡ marker thuộc target. Nếu denominator giảm làm over-cap sau khi đã gỡ marker trong target, actor có quyền phải chọn đủ marker ở ngoài target ngay trong cùng flow xác nhận; không tự động gỡ marker ngoài target.
...
- **D1 topic authorship amendment (2026-09-16; đã triển khai):** course authoring eligibility không tự biến user thành topic author. Mỗi topic có immutable original creator, đúng một responsible author và tối đa hai explicit contributors; original creator hoặc current responsible author được request/resubmit/withdraw, contributors chỉ edit, ngoài group read-only. Trước first successful approval, original creator/current responsible/initial contributors/pre-approval responsible users đều bị loại khỏi reviewer; responsibility transfer là topic operation riêng, role downgrade giữ nguyên authorship, còn remove/self-leave có affected responsibility phải transfer atomically cho recipient hợp lệ hoặc fail closed. Transfer feedback phải có topic/course context. Create navigation luôn dùng authoritative newly-created topic id, độc lập số lượng draft hiện có.
...
| --- | --- | --- | --- |
| D1 — Topic authoring → review → publication | **Outcome state:** D1 foundation, authorship amendment, v2/v4 corrections và follow-up đã merge qua PR #98 (`861e7c7`, head `d93385a`). Delivery status/evidence chi tiết thuộc `progress.md`; không suy ra deploy, remote DB hay full manual-QA matrix từ merge. | D1 sở hữu creator/responsible/contributor, group-scoped content edit, structure/delete/review capability riêng, rejection history/`review_notes`, lifecycle delete/restore và role-only downgrade so với remove/leave transfer. v3 target-attached note creation UI, email/push delivery và candidate revisions vẫn deferred. | Không còn follow-up implementation cho P0-A–P4-A; dùng canonical D1 detail plan cho invariant và acceptance hiện hành. Q7/D2 là boundary riêng, không nhập vào D1. |
| Q7 — Internal previewer access correction | **Outcome state:** Independent correction review `PASS`; PR #101 merge tại `65e8481`; production inventory, ba migration Q7, deployment đúng SHA và post-deploy checks đã hoàn tất. Evidence/giới hạn thuộc `progress.md`. | Course active có thể draft/pending/published, nhưng previewer-only chỉ đọc topic active/published kể cả media; giữ capability D1 độc lập. Ghi persistent progress/answer/review qua Action và Data API cần enrollment vào course của target; các relation/correctness invariants khác vẫn thuộc `LEARNING-INTEGRITY-001`. Đây là prerequisite authorization cho D2, không phải full invite/ownership rollout. | Bounded collaborator/access correction sau D1, trước D2; [Q7 detail plan](./implementation-plans/q7/plan.md) sở hữu lookup, policy, media và verification gates. |
| D2 — Preview topic contract | **Outcome state:** Đã triển khai, kiểm tra và independent review `PASS` ở local HEAD `39feb3a`; chưa push/PR/merge/deploy. Delivery evidence thuộc [progress.md](./progress.md); Owner Decision 12, `PREVIEW-001` và [D2 detail plan](./implementation-plans/d2/plan.md) giữ contract. | D1 publish-readiness và Q7 access correction đã là prerequisite được thỏa. Contract là course active/published + topic và chapter cha active, topic published/preview marker; full topic content gồm media, transient answer correctness, không persistent learning state. Quota là `ceil(active_topic_count * 20%)`, draft tính denominator; over-cap được xử lý theo lifecycle, moderation exception và inline resolution của detail plan. Dùng public preview read boundary và stateless answer-evaluation boundary riêng, không mở rộng collaborator read helper. | Workstream D2 hoàn tất cục bộ sau D1/Q7; không gộp collaborator invite/ownership. D3 là candidate kế tiếp theo working execution order, cần contract/authority riêng. |
...
9. PR D1: Topic authoring → review → publication — đã merge qua PR #98 (`861e7c7`); singular-owner invariant, authorship/lifecycle và các correction thuộc D1 đã vào `main`. Manual QA chỉ được claim theo các scenario đã ghi ở `progress.md`; không suy ra deploy hoặc remote DB mutation.
10. Q7: Internal previewer access correction — đã merge qua PR #101 (`65e8481`) và rollout production; xem `progress.md` cho bằng chứng cùng giới hạn xác minh.
11. D2: Preview topic — public readonly Preview sau D1 và Q7 đã hoàn tất, independent review `PASS` trên nhánh local; chưa push/PR/merge/deploy.
12. D3 Memory check — candidate kế tiếp theo working execution order; semantic contract vẫn cần chốt riêng trước implementation.
...
Mỗi Wave D candidate chưa triển khai cần implementation brief và Owner acceptance riêng trước khi code; D1 đã merge qua PR #98, Q7 đã merge/rollout qua PR #101, D2 đã hoàn tất và qua independent review trên nhánh local. [Canonical D1 detail plan](./implementation-plans/d1/plan.md), [Q7 detail plan](./implementation-plans/q7/plan.md) và [D2 detail plan](./implementation-plans/d2/plan.md) sở hữu contract tương ứng. D3 là candidate kế tiếp nhưng chưa được plan hoặc cấp implementation authority bởi mốc D2; D6 trở đi giữ deferred/open cho đến khi làm tới và có acceptance tương ứng.

## Wave D gates ở mức program

- D1 chỉ đạt khi normal course creation là teacher-only và tạo đúng một owner membership; DB không cho phép active owner thứ hai, invitation/normal role mutation chỉ dùng `co_owner` cho membership đặc quyền bổ sung; valid admin/student collaborator author được theo local membership và topic capability, previewer ngoài authoring group không sửa content còn previewer thuộc group được sửa content nhưng không reorder structure; admin/student không membership không normal-author; new topic không nhận user-selected `pending`/`published`, create đi thẳng vào builder ở `draft` và dùng đúng authoritative id dù có 0/1/nhiều draft cũ; mỗi topic có immutable creator, đúng một responsible author và tối đa hai contributors explicit, creator hoặc responsible được request/resubmit/withdraw, contributor chỉ edit, outside-group collaborator read-only, reject giữ responsibility, initial-review exclusion được bảo vệ trước first approval, role downgrade giữ authorship còn remove/self-leave có affected responsibility phải transfer atomic với recipient hợp lệ hoặc fail closed, transfer feedback có context, request review bị từ chối ở cả ba trạng thái thiếu content (rỗng, chỉ card, chỉ exercise), được phép khi có cả hai loại active content nhưng chưa publish, pending freeze được bảo vệ qua mọi normal content/status write surface, reviewer approve/reject obeys effective permission + no-self-review + reject reason, role/boolean-capability mutation không bỏ reviewer hợp lệ cuối cùng, fixed collaborator caps áp dụng cho invitation và role occupancy mutation với pending reservation, global `admin` role alone không review/publish nhưng vẫn có moderation/takedown/maintenance boundary riêng với mandatory audit, user self-delete profile bị chặn nhưng trusted admin maintenance path vẫn được bảo vệ, và published edit có confirm + atomic demotion về draft. Acceptance phải có action/schema/component, DB/RPC/RLS/direct-write, moderation-separation và concurrency evidence tương ứng.
- D1 review/lifecycle correction đã triển khai không còn rejection budget, escalation, rescue hoặc takeover; reject về `draft` với reason và cho creator/responsible resubmit theo current authority. `review_notes` có topic/card/exercise target và boundary đọc/ghi riêng; rejection history hiển thị theo lifecycle visibility, không coi response `0`/`[]` khi published là mất persisted history. Pending delete cancel submission rồi soft-delete atomically; published delete cần confirm và restore luôn về `draft`. Các chi tiết RPC, DTO, UI surface và evidence thuộc canonical D1 detail plan, không nhân đôi ở gate chương trình.
- Q7 chỉ đạt khi internal previewer đọc được course active ở draft/pending/published nhưng previewer-only chỉ đọc topic published + active kể cả media; quyền D1 authoring/review độc lập được giữ. Persistent progress/answer/review qua cả Server Action lẫn Data API phải cần enrollment thật vào course của target; previewer/collaborator đã enroll được ghi, còn role đơn thuần không cấp quyền. Acceptance phải bao phủ direct calls, INSERT/UPDATE RLS và media URL đã biết; Q7 không chứng minh các relation/correctness invariants còn lại của `LEARNING-INTEGRITY-001`.
