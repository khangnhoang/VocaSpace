...
- Current `get_topic_workflow_state` tính `canEdit` từ course role và `canRequestReview` từ `canEdit + draft + readiness`; `request_topic_review` cũng kiểm tra course authoring access, không kiểm tra responsible author. Vì vậy current D1 implementation chưa đáp ứng topic-level group, submitter restriction, initial-review exclusion hoặc transfer/leave invariant.
- Current `course_collaborators` role/capability RPC chỉ bảo vệ reviewer capability, role cap, last-reviewer safety và singular owner; removal/downgrade chưa transfer topic responsibility. Không có self-leave action, topic-authorship storage, append-only pre-approval responsibility evidence hoặc notification surface.
...
- P2 phải áp dụng topic authoring group cho mọi content mutation đã nằm trong D1 closure: responsible author và contributor được edit khi lifecycle cho phép; user ngoài group bị từ chối dù role course có authoring capability; `pending`, published-confirm và restore/delete semantics không đổi.
...
- Current evidence: RPC/action/component chain đã truyền exact returned id và không có draft selection code; missing proof là interaction-level regression. Implementation follow-up phải tái hiện bug trước khi sửa, sau đó test action result → router call và route arrival cho 0/1/multiple draft; không ghi root cause chưa được chứng minh thành fact.
...
7. A single atomic boundary for published content mutation + demotion to draft and published-topic soft-delete + demotion + soft-delete. It must cover card, exercise, question/group/option/media and metadata mutation families, or a common transaction primitive that all supported callers use. The same boundary must apply topic-group membership checks before content writes.
...
- Transfer/leave tests prove owner/co_owner removal or downgrade transfers responsibility atomically to an allowed recipient or blocks the mutation; self-leave warns per-topic, rejects missing recipients, and never randomly assigns. Transfer feedback includes course/topic context and self-take does not notify the actor.
...
- Topic builder/structure UI tests show responsible-author and contributor roles, read-only outside-group collaborators, transfer/leave warnings, rejection feedback, cap errors and discoverable transfer context without requiring visual redesign.
...
- **P4 — Closure:** focused/unit/component/integration/browser verification for the amended contract, accessibility/responsive/manual QA bounded to affected flows, singular-owner and topic-authorship invariant verification, self-review, docs/progress reconciliation and exact-head checkpoint report.
...
- Existing broad `is_admin()` use in content-read/management helpers remains an architectural coupling risk. D1 now separates normal course-thumbnail and question-group-media authoring from global admin moderation through course-scoped Storage/RPC checks; any future media surface still needs bounded course context for student/admin collaborators. Do not solve unrelated Q7/public access behavior or admin create-on-behalf in D1.
...
Implementation handoff readiness: **AMENDED CONTRACT — IMPLEMENTATION FOLLOW-UP REQUIRED**. The correction chain is committed locally as `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `43c8fcd`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df` and `0b27b85`; it preserves the earlier D1 boundary while closing the verified identity, Storage authorization/cleanup, invitation UX and singular-owner/UI findings. The 2026-09-16 topic-authorship and navigation amendment is accepted but not implemented; prior P0–P4 PASS evidence must not be presented as covering it. Reopen `P0-A → P1-A → P2-A → P3-A → P4-A`, with the create-navigation interaction regression included in P3-A. Remote migration/production remediation, push, pull request, merge and deploy remain outside authority. Application-only validation is not the guarantee; the amended implementation still requires a DB-backed trusted boundary plus application/read-model/UI consumers. Remaining evidence limits are recorded in `progress.md`/`problems.md`.
