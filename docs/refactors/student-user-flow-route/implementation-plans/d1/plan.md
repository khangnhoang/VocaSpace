...
Implementation handoff readiness: **AMENDED CONTRACT — P0-A/P1-A/P2-A/P3-A/P4-A PASS**. The correction chain is committed locally as `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df` and `0b27b85`; it preserves the earlier D1 boundary while closing the verified identity, Storage authorization/cleanup, invitation UX and singular-owner/UI findings. The 2026-09-16 authorship/navigation amendment is implemented through P0-A/P1-A/P2-A/P3-A/P4-A local checkpoints. P2-A adds the topic-group content boundary, closes legacy RPC-arity bypasses, aligns normal media upload authorization, and re-checks group membership after the mutation lock; P3-A adds the authorship workflow read/UI, persistent transfer feedback, collaborator summary/leave entry points, responsibility-safe collaborator mutations and authoritative create-to-builder navigation; P4-A adds current-route browser smoke evidence for zero, one and multiple existing drafts and closes the amended verification/self-review loop. Focused UI/action/schema verification is `5 files / 63 tests`, affected integration is `12 files / 127 tests`, full unit is `55 files / 463 tests`, browser smoke is `1 passed` with all three cardinalities, TypeScript and targeted ESLint pass. The existing legacy-route smoke remains follow-up debt, and full manual accessibility/responsive QA is not claimed. Prior P0–P4 PASS evidence remains historical for the earlier contract. Remote migration/production remediation, push, pull request, merge and deploy remain outside authority. Application-only validation is not the guarantee; the amended implementation requires a DB-backed trusted boundary plus application/read-model/UI consumers. Remaining evidence limits are recorded in `progress.md`/`problems.md`.
