...
P0-A đã cung cấp schema/provenance và P1-A đã chuyển group/request/transfer/leave/reviewer-exclusion qua trusted boundary; P2-A vẫn phải áp dụng cùng predicate cho toàn bộ content mutation closure.
...
Implementation handoff readiness: **AMENDED CONTRACT — P0-A/P1-A PASS, P2-A FOLLOW-UP REQUIRED**. The correction chain is committed locally as `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df` and `0b27b85`; it preserves the earlier D1 boundary while closing the verified identity, Storage authorization/cleanup, invitation UX and singular-owner/UI findings. The 2026-09-16 authorship/navigation amendment is now implemented through P0-A/P1-A local checkpoints; P2-A must still apply the topic group to all content mutation surfaces, followed by P3-A navigation/UX evidence and P4-A closure. Prior P0–P4 PASS evidence remains historical for the earlier contract and must not be presented as amended closure. Remote migration/production remediation, push, pull request, merge and deploy remain outside authority. Application-only validation is not the guarantee; the amended implementation requires a DB-backed trusted boundary plus application/read-model/UI consumers. Remaining evidence limits are recorded in `progress.md`/`problems.md`.
