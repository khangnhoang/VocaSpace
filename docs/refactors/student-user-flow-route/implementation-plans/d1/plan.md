...
Implementation handoff readiness: **AMENDED CONTRACT — P0-A/P1-A/P2-A PASS, P3-A FOLLOW-UP REQUIRED**. The correction chain is committed locally as `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df` and `0b27b85`; it preserves the earlier D1 boundary while closing the verified identity, Storage authorization/cleanup, invitation UX and singular-owner/UI findings. The 2026-09-16 authorship/navigation amendment is implemented through P0-A/P1-A/P2-A local checkpoints. P2-A adds the topic-group content boundary, closes legacy RPC-arity bypasses, aligns normal media upload authorization, and re-checks group membership after the mutation lock; focused verification is `18/18`, affected integration is `12 files / 127 tests`, full unit is `53 files / 456 tests`, TypeScript and targeted ESLint pass. P3-A still owns navigation/UX evidence and P4-A owns final closure. Prior P0–P4 PASS evidence remains historical for the earlier contract and must not be presented as amended closure. Remote migration/production remediation, push, pull request, merge and deploy remain outside authority. Application-only validation is not the guarantee; the amended implementation requires a DB-backed trusted boundary plus application/read-model/UI consumers. Remaining evidence limits are recorded in `progress.md`/`problems.md`.
