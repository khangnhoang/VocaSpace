...
- Authenticated Data API/RPC calls from admin/student collaborators prove membership-only authoring for topic/chapter/content; `previewer` remains denied. Media upload tests cover the course-scoped context required by authoring and deny admin-without-membership normal uploads. Ordinary question-group media DELETE is re-resolved through `course_id → topic_id`, uploader ownership and active topic-group membership, and is allowed only for an active `draft`; pending/published/removed/outside-group objects remain protected while the existing admin moderation exception remains available.
...
- **P2 — Content mutation safety:** route every supported topic/card/exercise/child mutation through topic-group authorization, pending guard and published confirm + atomic demotion; constrain ordinary question-group media DELETE to active draft topic-group uploader ownership while preserving the approved admin moderation exception; preserve admin approved maintenance/takedown semantics; cover soft-delete/restore, pending invalidation and rollback behavior without allowing outside-group course collaborators to edit.
...
Implementation handoff readiness: **AMENDED CONTRACT — P0-A/P1-A/P2-A/P3-A/P4-A PASS**. The correction chain is committed locally as `143102e`, `c56f3aa`, `65ed00d`, `6180356`, `2987794`, `301d0e1`, `e6c4d0e`, `d0d53df`, `0b27b85` and `98879ec`; it preserves the earlier D1 boundary while closing the verified identity, Storage authorization/cleanup, invitation UX, singular-owner/UI and media-delete lifecycle findings. The 2026-09-16 authorship/navigation amendment is implemented through P0-A/P1-A/P2-A/P3-A/P4-A local checkpoints. P2-A adds the topic-group content boundary, closes legacy RPC-arity bypasses, aligns normal media upload authorization, adds draft-only ordinary question-group media DELETE with uploader/group checks, and re-checks group membership after the mutation lock; P3-A adds the authorship workflow read/UI, persistent transfer feedback, collaborator summary/leave entry points, responsibility-safe collaborator mutations and authoritative create-to-builder navigation; P4-A adds current-route browser smoke evidence for zero, one and multiple existing drafts and closes the amended verification/self-review loop. Focused UI/action/schema verification is `5 files / 63 tests`, affected integration is `12 files / 127 tests`, full unit is `55 files / 463 tests`, browser smoke is `1 passed` with all three cardinalities; the later media-delete correction evidence is recorded in `progress.md`. The existing legacy-route smoke remains follow-up debt, and full manual accessibility/responsive QA is not claimed. Prior P0–P4 PASS evidence remains historical for the earlier contract. Remote migration/production remediation, push, pull request, merge and deploy remain outside authority. Application-only validation is not the guarantee; the amended implementation requires a DB-backed trusted boundary plus application/read-model/UI consumers. Remaining evidence limits are recorded in `progress.md`/`problems.md`.
