...
| D2 — Public topic preview | [d2/plan.md](./d2/plan.md) | — Owner decisions nằm trong master plan và detail plan | Kế hoạch chi tiết; implementation trên nhánh D2 riêng |

Các plan lịch sử trong `../plans/` được giữ tại chỗ cho PR đã hoàn tất. B3 đã được migrate vào hierarchy này; file B3 cũ không còn là một source song song. C1/C2/D1/Q7 là execution contract đã triển khai; D2 là contract cho workstream đang thực hiện. Delivery status/evidence vẫn thuộc [progress.md](../progress.md), risk/constraint thuộc [problems.md](../problems.md); index này không tự cấp implementation hay remote authority.
