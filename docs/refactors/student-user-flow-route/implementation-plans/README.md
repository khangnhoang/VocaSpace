...
Các plan lịch sử trong `../plans/` được giữ tại chỗ cho PR đã hoàn tất. B3 đã được migrate vào hierarchy này; file B3 cũ không còn là một source song song. C1 và C2 là execution contract lịch sử. Wave D planning đã mở D1 ở mức program contract trong `../plan.md`, `../progress.md` và `../problems.md`; detailed per-PR plan sẽ được tạo trong checkpoint kế tiếp. Owner request hiện tại là source cho D1 decision surface nên chưa tạo owner-review brief trùng lặp.
