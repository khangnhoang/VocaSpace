...
| D1 — Topic authoring → review → publication | [d1/plan.md](./d1/plan.md) | — Owner request là decision source; không tạo brief trùng lặp | Earlier P0–P4 + correction đã đạt local; topic-authorship/navigation amendment cần reopen `P0-A → P1-A → P2-A → P3-A → P4-A`; chưa push/PR |

Các plan lịch sử trong `../plans/` được giữ tại chỗ cho PR đã hoàn tất. B3 đã được migrate vào hierarchy này; file B3 cũ không còn là một source song song. C1 và C2 là execution contract lịch sử. D1 là detailed implementation contract; delivery status/evidence vẫn thuộc [progress.md](../progress.md), risk/constraint thuộc [problems.md](../problems.md), và Owner request hiện tại là source cho D1 decision surface nên không tạo owner-review brief trùng lặp.
