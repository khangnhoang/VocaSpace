...
The candidate artifact layout is `docs/ui-design-system-and-review/product-language.md`, `docs/ui-design-system-and-review/screen-types/<type>.md`, `docs/ui-design-system-and-review/components/<component>.md` when a separate component contract is justified, and `docs/ui-design-system-and-review/surfaces/<area>/<surface>.md`, with an index only when accepted designs exist to route to. These are future paths, not files to create in this checkpoint or a requirement to document every screen or component. `frontend-design` should own the conditional common-design authoring procedure and permanent discovery rule; the accepted design content should have one durable owner in `docs/ui-design-system-and-review/`. The eventual `frontend-ui-review` skill needs an explicit `AGENTS.md` activation route and a narrow handoff to the existing code-review lifecycle; any `docs/agent-loops.md` change must clarify that handoff without duplicating its verdict taxonomy.
...
## Program flow and workstream contracts

The semantic dependency DAG is `UI-1 → UI-2 → UI-4 → UI-5`, with `UI-2 → UI-3`. Each edge requires the relevant accepted output, not completion of every upstream artifact: the Teacher pilot needs the product language and TA type design, not an LE type design. An additional `UI-3 → UI-4` edge exists only if the pilot needs a changed shared-component contract; `UI-3 → UI-5` exists only if the reviewed pilot uses that change. UI-3 is not a requirement to finish the entire component system before a surface pilot. The `UI-4 → UI-5` edge gates UI-5 completion through a real pilot review; UI-5 authoring may begin once its accepted design inputs and evidence contract are stable.

| ID | Required input | Output contract and semantic owner | Consumer | Completion gate |
| --- | --- | --- | --- | --- |
| UI-1 | Owner acceptance of this plan's material architecture; current frontend skills and evaluations | LE/TA philosophy and always-on design-source discovery in `frontend-design`; conditional authoring guidance and affected evaluation evidence under skill-governance ownership | UI-2 and later design/implementation tasks | Both latitudes and emphases are unambiguous; accepted-source reuse works without loading authoring guidance for routine tasks; no routing, safety, or permission regression. |
| UI-2 | UI-1 routing; current tokens and real screens; Owner decisions on concrete visual samples | Owner-accepted product language, followed by separately accepted LE/TA screen-type designs that inherit it, in their respective `docs/ui-design-system-and-review/` sources; other types only when needed | UI-3, UI-4, UI-5, and later surface work | Palette, typography, surfaces, radius, motion, type-specific expression, and token mapping are explicitly accepted; adjectives alone do not freeze values. |
| UI-3 | Relevant accepted UI-2 decisions; shared-component usage audit | The smallest Owner-accepted shared-component design contract in the owning `docs/ui-design-system-and-review/` source and corresponding runtime variants when justified, starting with the `Button` decision; component code owns actual behavior | UI-4 and UI-5 only when the pilot uses changed variants; later relevant surfaces | Existing uses and responsive/accessibility effects are checked; approved semantics and dimensions map to code; focused verification passes. Other component contracts need demonstrated consumers. |
| UI-4 | Accepted UI-2 Teacher design and current route/permission contracts; UI-3 only when the pilot requires its changed shared contract | One Owner-accepted Teacher surface specification in `docs/ui-design-system-and-review/surfaces/` plus a runnable pilot in the owning feature code | UI-5 and later changes to that surface | The primary creation journey and direct item editing are observable; required states/component choices are explicit; implementation and targeted checks match the accepted surface design. |
| UI-5 | For authoring: accepted design inputs and a stable evidence contract. For pilot validation and completion: a runnable UI-4 candidate, fixtures, browser readiness, and the UI-3 contract if the pilot uses it | Routed `frontend-ui-review` procedure and an observed pilot UI-review result; the skill owns review method/evidence, while `code-review-and-quality` owns final integrated readiness | Code review's final verdict and later rendered UI reviews | `AGENTS.md` selects the skill; real pilot states, interactions, and viewports are inspected; contract mismatches and evidenced aesthetic/interaction findings remain distinct; pending checks stay visible. |
