...
| --- | --- | --- | --- |
| UI-1 | LE/TA philosophy and design-source routing | Implemented; semantic verification blocked | [Accepted detail plan](./implementation-plans/ui-1/plan.md); core/conditional reference, LE/TA philosophy, and five affected eval cases committed locally. Deterministic validation passed. A bounded eight-case CLI probe failed at reader startup because this Codex CLI account does not support `gpt-6-sol`; no evaluator or valid semantic graph ran. Do not claim UI-1 complete. |
