...
Planning hiện ở Stage 0 trên `feat/agent-skill-eval-cli-first`; implementation và live model/evaluator calls là gates riêng chưa được cấp quyền. Master plan không dồn toàn program vào một branch: `Stage 1 bounded-parallel CLI runner → Stage 2 prepare barrier/execution plan/concurrency estimate → Stage 3 exact reuse/resume/affected rerun → Stage 4 evaluator/report/CI/docs và separately authorized pilot`. Trong Stage 1, one-unit sequential vertical slice và bounded parallel proof nằm cùng branch/PR; sequential chỉ là diagnostic gate và không đủ để close stage. Separately authorized Stage 1 live gate bị giới hạn đúng `1` sequential reader rồi `2` distinct parallel readers, không có full sequential-then-parallel benchmark. Mỗi stage dùng independent branch/PR từ refreshed `main` sau khi stage trước merge.
