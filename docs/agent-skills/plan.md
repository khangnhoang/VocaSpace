...
Tài liệu này là authoritative cho historical foundation decision bundle PR 1, PR 2, split PR 3A/3B và structural-migration program được owner phê duyệt rõ ràng. Historical foundation PR 3B đã merge qua PR #62 tại `d8a67a1b1e015d44ab52095e823cd8334bf1fead`, vì vậy evaluation foundation đã complete. Phase kế tiếp trong [structural-migration-roadmap.md](./structural-migration-roadmap.md) gồm six program phases containing nine actual pull requests. Program-plan approval không tự cấp implementation hoặc later action permission.
...
### Owner-approved cho historical foundation PR 1
...
### Owner-approved cho historical foundation PR 2
...
### Owner-approved cho historical foundation PR 3

1. Tách thành hai implementation pull request thực tế: PR 3A sở hữu versioned suite schema, deterministic `validate` và `references/eval-design.md`; PR 3B sở hữu synthetic `prepare`, provenance, fixed workspace và deterministic `report`.
2. Dependency là `PR 2 → PR 3A → PR 3B → future consumer discovery`; PR 3 chỉ complete sau PR 3B merge và PR 3A không tự mở khóa real-skill pilot.
3. Foundation synthetic-only; repository-routing trong scope, native-trigger deferred; runner không invoke/grade model, spawn subagent, chọn winner, sửa candidate hoặc thực hiện Git/remote mutation.
...
PR 3A tạo concrete consumer cho `references/eval-design.md`: thiết kế/validate suite schema và runner-owned artifact contract. Core route reference bằng exact read condition; mandatory permission, safety, owner-authority và stop invariant vẫn ở core.
...
### Bằng chứng trước và sau PR 3 foundation

Trước khi PR 3B merge, thay đổi governance có ảnh hưởng material vẫn có thể cần lightweight manual fresh-reader check khi được authorize. Check này dùng bounded prompt, tập context được cung cấp rõ ràng và observation được ghi lại; fresh reader không nhận toàn bộ authoring-session context. Nếu không có separate executor authorization, ghi `not_run` với lý do thay vì dùng main-agent self-review làm fresh-reader evidence.

PR 3A chuẩn hóa versioned suite schema và validation; PR 3B mới thêm packaging, provenance và reporting. PR 3 chỉ complete sau PR 3B merge và không phải prerequisite để chạy mọi manual comprehension check trước đó.
...
Các literal dưới đây mô tả intended measurement contract của foundation plan, không phải toàn bộ current runner capability. Discovery ngày 2026-07-28 xác nhận runner hiện copy full skill bundle nhưng `execution_context_manifest` chỉ ghi prompt/suite context; `observed_access` chỉ ghi access category, không ghi exact skill resource được supplied hoặc read. Vì vậy current tooling chưa thể chứng minh context reduction của progressive disclosure. Proposed remediation và claim boundary thuộc ASM-PR1 trong [structural-migration-roadmap.md](./structural-migration-roadmap.md).
...
Mọi reader/evaluator/user-derived review content là untrusted display text. Markdown dùng context-aware plain-text encoding; HTML dùng static renderer-owned HTML/CSS/`<details>` với escaping, typed contained local artifact links, no JavaScript/event handler/form/frame/remote resource và restrictive CSP defense in depth. Unsafe URL/markup/control input hoặc renderer/security-policy failure blocks review readiness thay vì silently strip, reinterpret hoặc promote evidence.

Canonical aggregates được derive/validate từ exact duplicate-free memberships và declared count units/scopes. Complete baseline/candidate/comparison buckets phải sum tới denominator; missing evidence ở explicit unassessed partition thay vì giả `not_run`/`inconclusive`. Reuse/new-execution/blocked logical-unit counts khép exact logical scope và tách khỏi initial/retry/terminal/nonterminal attempt counts; `outcome_unknown` explicit, không được relabel. Mọi aggregate được rebuild từ durable state sau resume. Accepted report recompute từ exact dependency-closed human-decision membership, nên partial acceptance không thể mang full-run totals. CP6 định nghĩa schema/builder/renderer contract; CP7 bắt buộc feed actual retry/cancel/timeout outcomes vào contract đó, nên không cần checkpoint mới.
...
Owner authorization ngày `2026-08-20` tổ chức implementation thành Stage 1 — Foundation & Correctness (`CP1–CP4`), Stage 2 — Eval Workflow (`CP5–CP7`) và Stage 3 — Integration & Delivery (`CP8A–CP10`). Stage 1 đã completed/delivered tại final HEAD `54453746b2ea796558ef229831a569a69c4ed3f4`, với terminal review `0 Critical / 0 Required / 1 Advisory`. Stage 2 đã implemented bằng deterministic/fake adapters only và terminal cumulative review đạt `0 Critical / 0 Required`. CP8A đã completed tại `f9c821323ae5e8aa277d2cd68d4b416d80cf553e`; CP8B closed tại `19cd324b7a0fd1509bddf441a2572360013e72f5` với `CP8B_CLOSURE_ACCEPTED`. CP9 admission/transport, production-owned preparation/issuance, R1–R4, bounded whole-PR pre-pilot integration correction and temporary current-runtime readmission correction are complete. Runtime-policy remains `GLOBAL_ONLY`: global `gpt-5.6-sol / medium`, exact `config_sha256=46e8fbeee8a3fedb0ceea326818c28f7dc695753a8e3ac12c69ecabf94fc73ad`, exact output-schema hashes, explicit `turn_completion_timeout_ms=120000`, helper budget `0`, no escalation role, limits `15 / 12 / 0 / 27` and automatic retry `0`. The prior exact owner-authorized pilot was attempted once and stopped fail-closed at the first reader `thread/start`: write intent `1`, acknowledgement `0`, outcome unknown `1`, real `turn/start` and model/provider calls `0`. The terminal historical run is not resumable or retrospectively enriched. Two audit-blocking diagnostic-evidence semantics are corrected with fake-only evidence, and the remaining R2 compatibility finding is now corrected: current readers accept the exact retained `c03f159...` diagnostic shape under its historical semantics without synthesizing R2 certainty, while current R2 writes/validation remain strict. Owner decision ngày `2026-08-30` closes CP9 as `completed_with_no_go / architecture_superseded`: the attempt is retained as truthful failure evidence, not a successful pilot or production-readiness claim. CP10 cumulative review completed at `0 Critical / 0 Required`; PR #76 merged the workstream into `main` at `16bf80babd129fb42572603c7204e7c368aa3e67` after required CI passed. The separately planned CLI-first workstream now owns P0-complete preparation, independent unit execution, exact-success reuse, dependency-bounded rerun and unit-local recovery. Stage 1, Stage 2 và Stage 3 đã merge tuần tự qua PR #78/#79/#80; Stage 3 final head `d5d86b3` nằm trong merge commit `69e7235`, còn exact current verification/delivery state do `progress.md` sở hữu. App Server và native-subagent expansion không phải blocker và vẫn ngoài scope nếu chưa được approve riêng. Không có standing live-call authority. Grouping không đổi historical CP ownership hoặc rollback boundary.

## Agent Skill Eval Harness CLI-first

[CLI-first master implementation plan](./implementation-plans/eval-harness-cli-first/plan.md) ưu tiên một Codex CLI executor chạy được trước mọi backend expansion. Mục tiêu là validate toàn bộ selected static scope và prepare mọi reader trước reader call đầu tiên, finalize/validate dynamic evaluator input trước evaluator call tương ứng, giữ success theo independent unit, resume/retry unit-local failure và rerun dependency closure thay vì full run mặc định.

CLI program dùng fresh/ephemeral `codex exec` subprocess, reuse v1 synthetic packages và giữ một local one-writer state tối thiểu gồm unit identity, exact input/config hash, dependency edges và per-unit status. State này là functional prerequisite của resume/reuse, không phải security/attestation hardening.

Automatic exact reuse chỉ áp dụng khi exact model-visible input và dependencies của unit không đổi. Vì full `SKILL.md` hiện có thể là shared input của mọi reader, plan không semantic-guess rằng một line change chỉ ảnh hưởng một case. Vòng sửa nhanh dùng explicitly selected affected-only `patch-check`; prior results được giữ làm reference nhưng không bị relabel thành complete current-revision evidence. `max_attempts` là lifetime ceiling per logical unit trong một run, không reset theo revision/fingerprint; mọi spawn dùng cùng gate, exhausted work cần new run. Operational preflight luôn hoàn tất trước spawn nên error count là `0`; post-spawn failure giữ attempt-local terminal semantics.

App Server, native Codex subagent, generic multi-backend abstraction, provider-envelope/call-certainty proof, credential/network/tool-isolation proof, signed audit evidence, cleanup authority và distributed store đều ngoài CLI program. Internal invocation boundary vẫn bounded để một future workstream có thể thêm backend nếu owner quyết định; chúng không block CLI implementation.

Historical planning snapshot trước Stage 4 implementation: CLI-first Stage 0 đã merge qua PR #77 tại `e195569479ee49dd9592a93573c49ecad85cd9e6`; Stage 1 đã merge qua PR #78 tại `2616b052882d8c39a8a01e4da5167fd7d778f1df`; Stage 2 đã merge qua PR #79 tại `d4b5f78a09fa997da75e969822bbc678c7f002a4`; Stage 3 đã merge qua PR #80 tại `69e723556088cf885de1d7b2ebe4db29a44180f6`, chứa exact final head `d5d86b3681464d89d2d675b3e703ed34fe9a7f2a`. Historical pre-delivery snapshot giữ việc S3-CP0–S3-CP5 được implement/commit local trên `feat/agent-skill-eval-cli-reuse` và eleven supplied correction findings được xử lý trước delivery. Master đã yêu cầu cross-revision exact reuse từ Stage 0; accepted-attempt/producing-descriptor anchor được freeze trong master và [Stage 3 detailed plan](./implementation-plans/eval-harness-cli-first/stage-3-cli-reuse.md) dưới S3-CP0; historical Stage 2 plan không bị rewrite. Exact status/evidence hiện tại thuộc [progress](./progress.md), còn exact authority thuộc [CLI-first owner review brief](./implementation-plans/eval-harness-cli-first/owner-review-brief.md). Program order vẫn là `Stage 1 bounded-parallel CLI runner → Stage 2 prepare barrier/execution plan/concurrency estimate → Stage 3 exact reuse/resume/affected rerun → Stage 4 evaluator/report/CI/docs và separately authorized pilot`; [Stage 4 detailed plan](./implementation-plans/eval-harness-cli-first/stage-4-cli-evaluator-report.md) đã correct bốn owner-supplied `Required` findings ban đầu cùng một finding bổ sung, thêm bounded zero-unit regression, và correction re-review đạt `0 Critical / 0 Required` trên `feat/agent-skill-eval-cli-evaluator-report` từ exact Stage 3 merge; tại snapshot đó, plan đang chờ owner approval và chưa có Stage 4 implementation authority. Owner đã cấp implementation/local checkpoint policy sau snapshot này; quyền live/remote vẫn cần instruction riêng theo nguồn authority hiện tại.

The separately authorized single thread-only probe on the temporary `f4ba64aa...` admission received an exact matching `APP_SERVER_RPC_ERROR` (`-32600`) before acknowledgement because production projected sandbox literal `readOnly` while App Server `0.149.1` requires `read-only`. Write completed; response-channel and matching-response evidence are true; `turn/start`, model/provider calls and retries remain `0`. The probe authority is consumed. The bounded fake-only correction now preserves the internal policy and serializes exact `read-only` / `workspace-write` literals through the production thread request path; no new probe or live call occurred.
...
- behavior coverage tách thành ASM-PR2A, ASM-PR2B và ASM-PR2C;
...
```text
Owner đã duyệt historical foundation PR 1 governance decision bundle
  → historical foundation PR 1 governance contract
    → historical foundation PR 2 structural validator
      → historical foundation PR 3A versioned suite schema, validate và eval-design contract
        → historical foundation PR 3B synthetic prepare, provenance, fixed workspace và report (merged)
          → structural-migration discovery và owner approval (complete)
            → ASM-PR1 skill-resource evidence
              → ASM-PR2A frontend coverage + single CI validation step
                → ASM-PR2B validation/testing/data-safety coverage
                  → ASM-PR2C lifecycle/delivery-permission coverage
                    → ASM-PR3 frontend-design pilot
                      → ASM-PR4 product-engineering migration
                        → ASM-PR5A planning/review migration
                          → ASM-PR5B Git/GitHub migration
                            → ASM-PR6 isolated Supabase migration
```
...
### Historical foundation PR 3A — Versioned suite schema và validation

**Mục tiêu:** thêm suite-definition schema v1 cho regression/repository-routing/fresh-reader, deterministic `validate --skill|--all`, black-box Node tests, `eval-design.md` và documentation reconciliation.

**Phụ thuộc:** PR 2 đã merge qua PR #53; owner đã duyệt final PR 3A brief.

**Trong scope:** tooling/schema dưới `.agents/scripts/**`; future suite location contract `.agents/evals/<skill>/{regression,routing,fresh-reader}.json`; strict identity/path/policy validation; `suite_definition` và `validation_result` artifact types.

**Ngoài scope:** real suite, prepare/report, Git subprocess, provenance/workspace, model/subagent, semantic grading, skill creation/migration, package/CI/Vitest change.

**Acceptance:** existing unconfigured skill và zero-suite foundation state thành công đúng status; configured trio strict/deterministic; unsupported schema và operational refusal tách exit; executor/evaluator namespace tách; no absolute path leak; targeted local Node verification pass.
...
### Historical foundation PR 3B — Synthetic packaging, provenance và report (merged)
...
| --- | --- | --- |
| 1 | ASM-PR1 | Shared skill-resource access evidence |
| 2 | ASM-PR2A | Frontend experience coverage; 6 suite files; add the single CI `validate --all` step |
| 2 | ASM-PR2B | Validation, testing and data-safety coverage; 9 suite files; no CI change |
| 2 | ASM-PR2C | Lifecycle and delivery-permission coverage; 12 suite files; no CI change |
| 3 | ASM-PR3 | Controlled `frontend-design` pilot |
| 4 | ASM-PR4 | `frontend-workflow`, `test-quality-strategy`, `nextjs-server-action-zod` |
| 5 | ASM-PR5A | `implementation-planning-and-pr-breakdown`, `code-review-and-quality` |
| 5 | ASM-PR5B | `git-checkpoint-workflow`, `github-pr-ci-workflow` |
| 6 | ASM-PR6 | Isolated `supabase-safe-migration` and final reconciliation |

**Boundary:** exact exclusions, verification, fresh-reader, stop và rollback contract thuộc [structural-migration-roadmap.md](./structural-migration-roadmap.md). Material plan approval và quyền implementation/Git cho từng ASM-PR vẫn riêng biệt.

## Consumer hoặc migration sau foundation

Owner-approved roadmap xác định `frontend-design` là pilot sau ASM-PR1 và toàn bộ ASM-PR2A–ASM-PR2C coverage prerequisites. Pilot chưa được cấp implementation permission. Các skill permission-sensitive chỉ xuất hiện sau pilot gate; planning/review và Git/GitHub được tách thành ASM-PR5A/ASM-PR5B; `supabase-safe-migration` được cô lập ở PR cuối.
...
Fresh-reader base-versus-core-plus-references behavior testing là mandatory cho mỗi migrated skill trong ASM-PR3, ASM-PR4, ASM-PR5A, ASM-PR5B và ASM-PR6. ASM-PR1 và ASM-PR2A–ASM-PR2C chỉ dùng fresh reader khi materially useful; không chạy để inflate evidence.
...
Structural-migration ASM-PR2A thêm đúng một step `node .agents/scripts/run-skill-evals.mjs validate --all` vào existing appropriate Node 20 job. ASM-PR2B và ASM-PR2C chỉ thêm suites và sử dụng validation đã có; không duplicate hoặc sửa lại CI. Semantic fresh-reader execution không chạy trong CI.
...
```text
schema/tooling
suite definitions khi có real consumer được duyệt
optional concise owner-approved evidence summary tại later major gate
```
...
exact executor prompt/context package
baseline/candidate bundle copy
raw baseline/candidate observation
manifests, hashes và execution/access metadata
detailed human-evaluation working artifact
generated report và transcript khi có
workspace metadata
```

Không commit full raw evidence, secret/credential, environment dump, absolute temporary path, duplicated full skill snapshot, transcript hoặc disposable workspace.

Mọi committed schema có version. Foundation v1 không có cleanup command. OS-managed temporary storage không phải durable retention; evidence cần giữ cho owner gate phải được tóm tắt theo approved retention boundary trước khi temporary storage mất dữ liệu.

## Rủi ro và cách giảm thiểu

| Risk | Tác động | Mitigation và điểm phát hiện sớm nhất |
| --- | --- | --- |
| Draft wording ngụ ý permission | Unauthorized implementation hoặc Git action | Explicit status/authority wording và fresh-reader case; historical foundation PR 1 |
| Core safety bị chuyển sang optional reference | Permission/destructive regression | Mandatory core inventory và safety veto; historical foundation PR 1/ASM-PR3 pilot |
| Validator làm semantic judgment | False mechanical authority | Explicit authority boundary và tests; historical foundation PR 2 |
| Frontmatter subset biến thành YAML parser | Complexity/compatibility defect | Strict v1 grammar; historical foundation PR 2 |
| Runner biến thành hidden model workflow | Quota, nondeterminism, permission expansion | Synthetic-only validation/packaging/reporting; historical foundation PR 3A/3B |
| Baseline/candidate không equivalent | Invalid improvement claim | Provenance manifest và `inconclusive`; historical foundation PR 3B |
| Requested policy bị report như enforced | False safety evidence | Record actual enforcement source; historical foundation PR 3A contract + PR 3B artifact |
| Allowlisted package bị hiểu là sandbox | Host/repo access vẫn có thể xảy ra | Explicit non-sandbox vocabulary và `not_run`; historical foundation PR 3A contract + PR 3B report |
| Context giảm nhưng behavior giảm | Unsafe rollout | Safety veto và invariant-driven future consumer gate |
| Current status bị duplicate | Future agent dùng stale state | Plan/progress ownership; mọi PR |
| Future consumer lan thành broad migration | Khó rollback, evidence trộn lẫn | Separate consumer discovery và owner gate |
...
### Đã được owner duyệt cho historical foundation PR 1
...
### Đã được owner duyệt cho historical foundation PR 3

- Split PR 3A/3B và dependency/completion boundary.
- Suite schema, validation status, executor/evaluator split và artifact identity boundary.
- Synthetic-only runner contract, authority split và no-model/no-semantic-grading rule.
...
- ASM-PR1 shared resource-access evidence scope.
- Coverage split ASM-PR2A/ASM-PR2B/ASM-PR2C với 6/9/12 suite files; CI `validate --all` chỉ thêm ở ASM-PR2A.
- `frontend-design` pilot; ASM-PR4 product-engineering; ASM-PR5A planning/review; ASM-PR5B Git/GitHub; isolated ASM-PR6 Supabase.
...
- ASM-PR1 shared evidence tooling complete một lần.
- ASM-PR2A/ASM-PR2B/ASM-PR2C đã commit đủ 6 + 9 + 12 suite files cho cả chín candidates; CI validation chỉ được introduce một lần ở ASM-PR2A.
- ASM-PR3 pilot pass explicit owner continue gate trước multi-skill rollout.
- ASM-PR4, ASM-PR5A và ASM-PR5B giữ per-skill baseline, migration, comparison, correction và rollback boundaries.
- ASM-PR6 chỉ migrate Supabase và final-reconcile toàn bộ chín migrations cùng hai unsplit decisions.
