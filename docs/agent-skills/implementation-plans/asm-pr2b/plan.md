...
| `ctx-payment-schema` | `ctx-payment-schema` | `repository_file` | `lib/schemas/payment.ts` | Freeze current payment boundary schema. | `nsaz-reg-schema-type-ownership`. |
...
| `ctx-payment-action` | `ctx-payment-action` | `repository_file` | `app/actions/payment.ts` | Freeze current payment action/provider boundary. | `tqs-reg-mock-regression-determinism`, `tqs-route-provider-mock-regression`, `tqs-fresh-mock-regression-header-overlap`, `nsaz-reg-schema-type-ownership`, `nsaz-route-db-rpc-contract`, `ssm-route-rpc-validation-contract`. |
