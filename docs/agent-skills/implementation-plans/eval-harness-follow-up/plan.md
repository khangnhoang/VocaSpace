...
- Quyền hiện tại: local source implementation và local checkpoint commits theo đúng thứ tự/điều kiện trong plan đã được authorize bởi instruction hiện tại. Owner đã cấp và đã tiêu thụ một lần exact live/egress authority cho S4-CP4 theo [canary manifest](./canary-manifest.md); không còn standing live/model/calibration authority. **Push, PR và merge vẫn chưa được authorize.**
...
Implementing agent đọc plan + owner brief + current progress + relevant skills, kiểm tra branch/base/diff, bắt đầu CP1 rồi CP2, report checkpoint trước commit. CP3 chuẩn bị/review canary sau implementation; S4-CP4 là live pilot riêng, chỉ chạy sau exact owner grant và phải giữ stop rule. Permission cho source implementation và từng local commit đến từ instruction hiện tại; push/PR/merge vẫn là quyền riêng.
...
Plan decision là `frozen / owner-approved`; self-review xác nhận không có contract-level contradiction với branch/repo hiện tại. CP1 và CP2 implementation cùng deterministic verification đã hoàn tất sau khi tiếp quản partial diff; CP3 đã prepare exact canary manifest và S4-CP4 live outcome được ghi ở dưới. S4-CP4 kết thúc `incomplete` theo stop rule; không retry/resume Luna hoặc mở rộng calibration/fanout. Push/PR/merge chưa được cấp. Confidence về phạm vi và discovery `High`; implementation safety của CP1/CP2 được xác lập sau tests, hygiene và main integration review đã liệt kê.
...
- Verification at the initial CP1 checkpoint: targeted CP1 `9/9`; full `.agents/scripts/run-skill-eval-cli.test.mjs` `105/105`; changed `.mjs` `node --check`; targeted ESLint `0 errors`; `git diff --check` pass. The later integrity-matrix review found that this evidence did not cover the composed cross-run donor path completely; the original `0 Critical / 0 Required` claim is superseded by the correction checkpoint below. Fake child/local temp fixtures only; no live/model/provider/evaluator call.
...
- Finding disposition: `confirmed / Required / in scope`. The frozen integrity matrix required composed donor-path evidence for schema-valid substitutions, publication/recovery windows, attempt ceilings, command eligibility, same-ordinal cross-run separation, and byte-stable reads; the initial nine CP1 tests did not prove all of those guarantees after enrollment. This is an evidence gap, not a frozen-plan conflict.
- Correction: added table-driven tests from two independent valid donor graphs for run/unit/role/attempt/record/result/producing-plan/output substitutions; donor-root/artifact symlink and recovery-only late-result cases; marker/next-revision partial publication states; invalidation lifetime ordinal/budget progression; retry/resume/patch-check/unknown no-reimport behavior; same-attempt-ID separation; and donor/recipient `status`/`report` byte snapshots. Added the missing manifest-to-plan run identity and unit filename/inventory guards needed by the exercised relationships. No evaluator cross-run reuse, fingerprint change, dependency, or scope redesign.
- Verification: targeted `node --test --test-name-pattern '^CP1 ' .agents/scripts/run-skill-eval-cli.test.mjs` passed `35/35`; full `node --test .agents/scripts/run-skill-eval-cli.test.mjs` passed `131/131` with sandbox escalation for fake-child/temp filesystem access; changed `.mjs` `node --check`; targeted ESLint `0 errors`; `git diff --check` pass. Fake child/local temp fixtures only; live/model/provider/evaluator calls `0`.
- Correction is a separate local commit and does not amend `da72752`. CP2/CP3, live canary, push/PR/merge remain unperformed and unauthorized.

## CP2 actual checkpoint — 2026-09-07

- Diff disposition: `sound/continuable`; review lại từ `a28eae6` xác nhận không có plan/contract conflict và không discard previous-worker changes. Bổ sung hardening trong cùng scope: reader legacy literal độc lập với evaluator constant, strict prepared-option validation, safe model control-character rejection, positive revision identity và frozen reader options qua low-level revision publication/recovery.
- Implemented configurable reader flow: `prepare` nhận `--reader-model`/`--reader-effort` cho new run, freeze chung baseline/candidate trong plan v3 và exact descriptor projection, worker dùng đúng prepared options để build argv, evaluator vẫn Sol/medium, historical plan v1/v2 replay giữ legacy options/bytes, donor model/effort mismatch không import và `prepare --run` kế thừa config.
- Tests: fake child/local temp tests chứng minh options → descriptor → argv cho cả reader roles, two-sided parity, default/historical replay, restart/revision, donor mismatch/match, duplicate/unknown/malformed flags, same-run override rejection và low-level revision guard. Không có live/model/provider/evaluator dispatch.
- Verification: targeted `node --test --test-name-pattern '^CP2 ' .agents/scripts/run-skill-eval-cli.test.mjs` passed `6/6`; full `node --test .agents/scripts/run-skill-eval-cli.test.mjs` passed `137/137`; `node --test .agents/scripts/run-skill-evals.test.mjs` passed `130/130`; `node --test .agents/scripts/validate-skill.test.mjs` passed `37/37`; `node .agents/scripts/validate-skill.mjs` returned `valid`; `node .agents/scripts/run-skill-evals.mjs validate --all` returned `valid`; changed `.mjs` `node --check` and targeted ESLint passed `0 errors / 0 warnings`; `git diff --check` pass. Fake child/local temp only; live/model/provider/evaluator calls `0`.
- Scope review: no fingerprint algorithm, evaluator cross-run reuse, dependency graph, report subsystem, package dependency, product/UI/DB behavior or canary authority changed. CP3 canary, live/model, push/PR/merge remain pending/unauthorized. Frozen CP2 message: `feat(skill-evals): configure reader model independently of evaluator`.

## CP3 actual checkpoint — 2026-09-07

- CP3 không sửa product/harness source. Exact canary manifest/grant request đã được prepare tại [canary-manifest.md](./canary-manifest.md), pin pair B/C cụ thể cho owner review, bốn case mặc định, hai arm Sol/medium và Luna/max, evaluator Sol/medium, exact package/input/schema hashes, run/workspace roots và closure commands.
- Deterministic current-head checks: `validate --all` trả `valid` với `9` skills / `27` suite files / `187` cases; cả hai `prepare` trả `prepared` với dispatch `0`; `status` đọc thành công với `44` reader pending và `22` evaluator dependency-blocked mỗi arm; `report` giữ `incomplete`/exit `1` vì chưa dispatch closure nào. Run tree và package workspace tree byte-identical trước/sau các lệnh đọc; cross-arm package/input/evaluator/schema audit pass.
- Tại thời điểm CP3 commit, status là `prepared_for_owner_review`; live authority, billing/auth observation, model/provider/evaluator calls và egress chưa được cấp/quan sát. Không chạy model probe hoặc canary closure trong CP3; prepared run IDs không tự cấp quyền dispatch.
- Suggested documentation checkpoint message: `docs(skill-evals): finalize Luna canary grant manifest`. Chưa stage/commit trong checkpoint này; commit chỉ thực hiện sau owner approval theo Git checkpoint gate.

## S4-CP4 live canary outcome — 2026-09-07

- Owner đã cấp exact one-time live/egress authority cho đúng manifest: B=`2be02df11e279b5c88f37d2fd609069a54c235ed`, C=`af732c235d0955f6dafb17718a0cbe34f858b01d`, package `supabase-safe-migration`, bốn case, hai arm `gpt-5.6-sol/medium` và `gpt-5.6-luna/max`, evaluator `gpt-5.6-sol/medium`, `max_attempts=1`, retry `0`, automatic retry `0`, concurrency `2`, ceiling `12` dispatch/arm. Chỉ dùng các closure command trong manifest; không full-suite, fanout, replacement, resume hoặc candidate edit.
- Prepared run/revision: Sol `run-dbcc351f90b24a2f9a172b63841b2e27`, Luna `run-30bbc8bb9bca4963a4ad50b56ea2989d`, revision `1`. Sol chạy đủ bốn closure, `12/12` dispatch settled `succeeded`, `0` failed/unknown/integrity. Luna chạy route và fresh closure, sau đó additive reader closure phát sinh hai `outcome_unknown`; theo stop rule không chạy seed closure.
- Actual dispatch tổng cộng `20/24`: Sol `8` readers + `4` evaluators; Luna `6` readers + `2` evaluators. Mọi attempt là ordinal `1`, không có retry hoặc unit ID ngoài manifest. Luna additive readers `reader-96bfb0e1b1ae1dfb24a3cc967e8fe9f6b8df5a32f6601c4c0086cef4f50283fe` và `reader-6a35d40b0c421b30c6cc5b8b3a824a9a4088d2dd82c455ad446f4e9b23b21fba` đều `process_outcome_unknown` sau lần lượt `120043 ms` và `120058 ms`; không có structured output hoặc usage nên không suy diễn provider outcome/schema.
- Selected-graph coverage: Sol cả bốn case là `current`; rubric lần lượt `partially_satisfied`, `satisfied`, `partially_satisfied`, `satisfied`. Luna route/fresh là `current` và `partially_satisfied`; additive là `incomplete` do hai unknown và evaluator không chạy; seed là `incomplete` vì closure chưa chạy. Fresh case ghi nhận permission-quality regression của Luna so với Sol: Luna không yêu cầu cấp quyền còn thiếu một cách rõ ràng, dù không có remote mutation và safety veto không bị trigger trong closure đó.
- Raw evidence giữ ngoài tracked source tại `C:\Users\dongu\AppData\Local\Temp\vocaspace-agent-skill-evals\cli-v1\run-dbcc351f90b24a2f9a172b63841b2e27` và `C:\Users\dongu\AppData\Local\Temp\vocaspace-agent-skill-evals\cli-v1\run-30bbc8bb9bca4963a4ad50b56ea2989d`. Mười tám settled outputs đều `schema_version=1`, exit `0`, có usage; hai unknown không có structured output/usage. Tổng usage settled: Sol `261631` input / `10087` output / `4853` reasoning; Luna `113795` input / `7443` output / `6231` reasoning, cached input `11008`. Không có schema failure; stderr chỉ có warning PowerShell shell snapshot đã biết.
- Raw process latency: Sol `393542 ms` tổng (`316407 ms` readers, `77135 ms` evaluators; trung bình lần lượt `39551 ms` và `19284 ms`); Luna `421853 ms` tổng, trong đó `181752 ms` settled và `240101 ms` là hai timeout unknown (`383709 ms` readers attempted, `38144 ms` evaluators; settled trung bình toàn arm `30292 ms`). Đây là process duration quan sát được, không phải provider latency hay dollar cost.
- CP4 kết thúc `incomplete` theo contract. Không đủ evidence cho toàn bộ Luna graph và có permission-quality regression, nên không có calibration expansion/default change. Reader default tiếp tục `gpt-5.6-sol / medium`; không retry/resume Luna, không full calibration/fanout. Live authority đã tiêu thụ; push/PR/merge vẫn là quyền riêng và chưa được cấp.
