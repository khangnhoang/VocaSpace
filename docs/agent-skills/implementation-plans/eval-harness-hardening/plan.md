...
- Current authority permits exactly the bounded CP9 runtime model-policy feasibility/correction pass and one local coherent commit after terminal `0 Critical / 0 Required`. It does not permit the CP9 live pilot, push, PR creation/update, CI watch/fix, merge, deployment, history rewrite or CP10.
...
The behavior-relevant admission contract changes because it now binds precise output-schema hashes and the explicit completion window. The observed runtime fingerprint remains unchanged: `GLOBAL_ONLY`, `gpt-5.6-sol / medium`, `config_sha256=6f38a9f22d10b5fa430637ce5be6a586f5728c000727141afb20be2a4f79bcf4`; limits remain `15 reader / 12 evaluator / 0 helper / 27 total`, automatic retry remains `0`. Live authorization must be reissued for the exact corrected repository/preparation identity after independent re-audit.
