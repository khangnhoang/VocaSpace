...
- CP9 admission/measurement design, bounded live transport, production-owned preparation, bounded R1–R4 correction and R4 regression-semantics follow-up are complete. The runtime-policy feasibility pass concludes `GLOBAL_ONLY`: current run/readiness/dispatch ownership has one runtime hash, so role-specific binding would require out-of-scope architecture work. New preparation is admitted only for global `gpt-5.6-sol / medium` with exact `config_sha256=6f38a9f22d10b5fa430637ce5be6a586f5728c000727141afb20be2a4f79bcf4`; the previous `high`/`349a...` pair fails before materialization. Reader/evaluator use the global runtime; dormant helper inherits that policy but remains unauthorized and budget `0`. No explicit escalation role exists and none was added. Complete invocation/readiness closure remains validated before authority issuance, and resolved authority remains cross-bound to the preparation-owned grant before mutation. The pilot remains `not_run` and requires separate live authorization after independent re-audit; push, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
