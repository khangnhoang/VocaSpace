...
- CP9 admission/measurement design, bounded live transport and the production-owned preparation correction are complete. The behavior-runtime readmission now durably freezes exact `config_sha256=349a383d4d348c48288cea738b61f2dbcebb0bb32ba5cc1c55150aa38934b60d` from a production non-model App Server preflight with `account_type=chatgpt`, `thread_creation=not_started`, `turn_dispatch=not_started` and `model_calls_dispatched=0`; this supersedes the earlier implicit/unfrozen configuration state without reconstructing a historical value. The fingerprint permits completion of the bounded R1–R4 correction only. The pilot remains `not_run` and still requires separate live authorization after that correction; push, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
...
5. The authorized CP9 Tier 1 attempt stopped before `reader-canary`; no authority record or model turn was created. Its production preparation gap is corrected and deterministically certified, and the subsequent behavior-runtime readmission freezes exact `config_sha256=349a383d4d348c48288cea738b61f2dbcebb0bb32ba5cc1c55150aa38934b60d` with zero thread/turn/model dispatch. CP9 remains `not_run`; the bounded R1–R4 correction must be completed and reviewed before a separate exact live authorization can permit pilot execution.
