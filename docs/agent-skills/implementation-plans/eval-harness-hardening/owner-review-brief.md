...
- CP9 admission/measurement design, bounded live transport and the single required preparation correction are complete. `cp9 prepare --executable <exact-standalone-path>` now owns deterministic admitted task/run/readiness/four-plan materialization, exposes no arbitrary plan/run/grant inputs, and performs only a zero-thread/zero-turn preflight. Exact grant construction and issuance remain separate and reuse `issueLiveDispatchAuthority(...)`; `cp9 live` rejects any plan not owned by the canonical preparation record. The failed pre-dispatch attempt remains historical evidence with calls `0`; the pilot still requires new readmission and separate live authorization. Push, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
...
5. The authorized CP9 Tier 1 attempt stopped before `reader-canary`; no authority record or model turn was created. Its single Required preparation gap is now corrected and deterministically certified with terminal boundary review `0 Critical / 0 Required`. CP9 remains `not_run` and must pass a new bounded readmission plus separate exact live authorization before any pilot execution.
