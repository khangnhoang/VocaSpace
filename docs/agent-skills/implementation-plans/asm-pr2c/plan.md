...
## 17. Current semantic-contract correction
...
Current deterministic verification: focused validators pass `18/20/21/24`, each with `0 diagnostics`; cumulative `validate --all` passes `9 skills / 27 files / 177 cases / 0 diagnostics`; `validate-skill.mjs` is `valid` for 11 skills with `0 errors / 4` existing non-blocking `CORE_LENGTH_SIGNAL` warnings; `git diff --check` exits `0` with only working-copy LF→CRLF warnings. All 14 explicit semantic audits pass, including exact `12/83`, `38/25/20`, 83/83 frozen prompts/IDs, 25/25 negative routing vetoes and exact classifications, 83/83 physical-reference ownership and package sufficiency, 18/18 explicit missing-evidence stops, six/six non-small-safe CI stops, `53/53` contexts, durable head-scoped truth, and exact 10-file correction scope.
