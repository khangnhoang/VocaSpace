...
Owner instruction ngày `2026-09-04`: “thực hiện detail plan asm pr6” duyệt detailed plan hiện tại và cấp local implementation theo đúng checkpoints. CP0 đã hoàn tất; CP1 structural implementation đã verified, chưa commit. CP2 chờ immutable `C_struct`; CP3/CP4 chưa thực hiện. Stage/commit, live/model/egress/retry, push, PR, CI, merge và database actions vẫn cần quyền riêng theo plan. Những trạng thái `pending` trong evidence planning cuối tài liệu là lịch sử trước instruction này.
