...
- Coverage correction được owner chốt ngày `2026-09-04` sau advisory verdict drift của case A: cập nhật `question_options` product contract và exact eval context/rubric, đồng thời ghi general soft-delete ordering requirement vào ASM-PR6 backlog. Đây là separately authorized suite correction theo DB-coverage stop rule, không đổi CLI behavior/checkpoint, không sửa skill hoặc historical migration, không relabel pilot proposal thành human acceptance và không cấp live/ASM-PR6/remote authority.
- Owner chốt Stage 4 tooling ngày `2026-09-04` sau corrected six-call pilot và riêng một two-call run trên coverage correction `35cc5a1`. Đủ evidence để đóng implementation/tooling và chuẩn bị migration; không phải semantic acceptance của Supabase hoặc production certification. [Closure và provenance](../../progress.md#7-stage-4-tooling-closure-và-migration-handoff-2026-09-04) giữ các giới hạn này; checkpoint, schema, lifecycle và frozen Stage 3 semantics không đổi. Quyền docs-only commit/push/PR lần này thuộc owner brief, không phải standing authority từ master.
