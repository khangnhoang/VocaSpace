...
- Plan status: `owner-approved contract / S4-CP1–S4-CP3 committed / S4-CP4 tooling gate complete / Stage 4 tooling closed / semantic migration acceptance not established`. Initial pilot failure và các correction/pilot sau đó giữ nguyên historical evidence tại [progress — S4-CP4](../../progress.md#s4-cp4-pilot-fail-và-schema-correction-2026-09-03); [closure record](../../progress.md#7-stage-4-tooling-closure-và-migration-handoff-2026-09-04) ghi riêng latest two-call run trên `35cc5a1`. Tooling closure không phải Git merge hoặc whole-suite semantic pass.
- Historical implementation authority: owner đã yêu cầu implement theo master/detail, approve report-exit correction và authorize local checkpoint commits tuần tự sau deterministic verification + formal review `0 Critical / 0 Required` khi diff tạo coherent, meaningful commit boundary. Tại snapshot đó không có push/PR/merge authority. Current owner grant chỉ chốt docs, commit, normal push và tạo Stage 4 PR theo [owner brief](./owner-review-brief.md); không thêm live calls, sửa skill/source, CI self-fix hoặc merge. Không commit empty/no-op hoặc unresolved findings; không amend/squash.
