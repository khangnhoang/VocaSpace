...
- Plan decision status: Stages 1–3 `completed and merged`; Stage 4 master/detail đã được owner chọn làm implementation contract. Report-exit correction committed `8f424cd`; S4-CP1 committed `e01bb05`, S4-CP2 committed `9e126b6`, S4-CP3 committed `8ba4526`, mixed-budget correction committed `d5344da`, schema correction committed `0e92e93`, đều sau required verification/review. Initial S4-CP4 `stopped_on_evaluator_failure`; corrected S4-CP4 tooling pilot completed đúng `6` live calls, còn semantic migration acceptance chưa được thiết lập. Xem [progress — S4-CP4](../../progress.md#s4-cp4-pilot-fail-và-schema-correction-2026-09-03). Các planning decisions trong master là historical records; current action authority nằm tại mục này và progress.
...
- Historical implementation authority: owner authorize Stage 4 sequential implementation và local checkpoint commit khi deterministic verification + formal review đạt `0 Critical / 0 Required` và diff có coherent, meaningful boundary. Correction phải fix/re-review trước commit; không commit empty/no-op, không amend/squash lịch sử. Tại snapshot đó, không push, PR, merge hoặc remote action; live/model/evaluator call cần grant riêng cho S4-CP4.
- Historical initial grant ngày `2026-09-03`: hai case Supabase candidate-only, tối đa `6` dispatch, concurrency `2`, frozen `max_attempts=2`; owner approve egress qua Codex/OpenAI sau initial automatic approval rejection ở zero dispatch. Đã dùng `4` dispatch (`2 reader` succeeded, `2 evaluator` failed), stop trước reuse/affected rerun, retry `0`. Hai dispatch chưa dùng không mở quyền recovery sau stop. Tại thời điểm đó, authority chỉ gồm schema correction/tests/history/local checkpoint; replacement pilot cần grant mới.
- Corrected grant ngày `2026-09-04`: exact hai packages trên qua Codex/OpenAI `gpt-5.6-sol`, tổng `6`, concurrency `2`. Run mới dùng đủ `3 reader + 3 evaluator`; initial, same-ref reuse và A-only affected rerun đều đạt transport/state assertions. Grant đã consumed, không còn standing live authority. Case A final advisory proposal `partially_satisfied / triggered`; proposal đó không có human authority. Sau semantic investigation, owner đã chốt riêng `question_options` product contract và authorize coverage correction cho ADR/eval/backlog/history; quyết định này không chọn pilot winner, không xác nhận ASM-PR6 migration, không cho sửa skill/historical migration và không cấp live hoặc remote action.

- Current closure/authority ngày `2026-09-04`: owner chốt Stage 4 implementation/tooling, cho phép docs-only closure, commit sau review, normal push nhánh Stage 4 và tạo PR vào `main`. Grant này bao gồm delivery các Stage 4 commits đã có; không authorize source/skill/suite/SQL changes mới, thêm live calls, CI self-fix, merge, deployment hoặc ASM-PR6 implementation. Trạng thái delivery tại checkpoint docs là pending; Git/GitHub và báo cáo sau thao tác xác nhận kết quả, không coi grant là đã push/PR open.
- Post-correction single-case run `run-25d2635a48444d8388e750cb4f288cd3` tại `35cc5a1` đã dùng riêng `1 reader + 1 evaluator`, concurrency `1`, `max_attempts=1`, retry `0`; cả hai succeeded. Grant live này đã consumed. Proposal vẫn `partially_satisfied / triggered`; `current` không phải human acceptance. Hai nhận xét ambiguous-data gate và restore boundary cần xác minh trong migration review, không mặc định là confirmed skill defect. Không cần thêm live call để đóng tooling; migration preparation không đồng nghĩa skill migration được nghiệm thu. Xem [closure evidence](../../progress.md#7-stage-4-tooling-closure-và-migration-handoff-2026-09-04).
...
| Stage 4 | `feat/agent-skill-eval-cli-evaluator-report` | Evaluator/report/CI/docs; tooling closed, semantic migration acceptance separate | Historical failed initial batch `4`; corrected grant `6/6`; post-coverage single-case grant `2/2`; no further live authority |
...
- [x] Draft, correct bốn owner-supplied `Required` findings ban đầu cùng một finding bổ sung và bounded correction re-review Stage 4 detailed implementation plan trên exact merged Stage 3 source; không thêm checkpoint/persisted schema/lifecycle, không đổi frozen Stage 3 semantics và không chạy model.
- [x] Owner chọn master/detail làm Stage 4 implementation contract, approve report-exit correction và cấp sequential local checkpoint commit policy; live/delivery authority vẫn tách riêng.
