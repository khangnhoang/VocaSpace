...
- Grant S4-CP4 ngày `2026-09-03`: hai case Supabase candidate-only, tối đa `6` dispatch, concurrency `2`, frozen `max_attempts=2`; owner cũng approve egress qua Codex/OpenAI sau initial automatic approval rejection ở zero dispatch. Đã dùng `4` dispatch (`2 reader` succeeded, `2 evaluator` failed), stop trước reuse/affected rerun, retry `0`. Hai dispatch chưa dùng không mở quyền recovery sau stop. Current authority chỉ gồm schema correction/tests/history/local checkpoint theo policy; live retry hoặc replacement pilot cần grant mới.
...
| Stage 4 | `feat/agent-skill-eval-cli-evaluator-report` | Evaluator/report/CI/docs and migration pilot gate | Historical grant `6`, used `4`; stopped, no further live authority |
