...
- Corrected grant ngày `2026-09-04`: exact hai packages trên qua Codex/OpenAI `gpt-5.6-sol`, tổng `6`, concurrency `2`. Run mới dùng đủ `3 reader + 3 evaluator`; initial, same-ref reuse và A-only affected rerun đều đạt transport/state assertions. Grant đã consumed, không còn standing live authority. Case A final advisory proposal `partially_satisfied / triggered`; owner/human semantic decision, migration implementation và mọi remote action vẫn chưa được cấp.
