...
- Authority: the current task authorizes exactly the bounded CP9 live transport/launch correction and its two local commits. Live pilot calls, push, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
- Stage status: Stage 1 CP1–CP4 is `completed / delivered`; final delivered Stage 1 HEAD is `54453746b2ea796558ef229831a569a69c4ed3f4` and terminal review is `0 Critical / 0 Required / 1 Advisory`. Stage 2 CP5–CP7 is `completed / delivered` with terminal cumulative review `0 Critical / 0 Required`. Stage 3 CP8A is `completed` at `f9c821323ae5e8aa277d2cd68d4b416d80cf553e`; CP8B is `CP8B_CLOSURE_ACCEPTED` at `19cd324b7a0fd1509bddf441a2572360013e72f5`; CP9 transport correction is implemented but live admission remains `STOP` because App Server launch did not reach protocol readiness; CP10 is pending.
...
- CP9 admission/measurement design: historical generations remain separate. ASM-PR3 full-fanout reader usage is `61` (`reconstructed` from `18 + 1`, `36 + 2`, and `4` durable observations); semantic-substitution dogfood is `62 reader / 0 helper / 0 evaluator` (`exact_observed`). Historical model/runtime/tokens/wall-clock and complete model-visible input/output bytes are `unavailable`. Tier 1 selects the real staged ASM-PR5B range `3fa621c86399e5c1a9e43bd9cd7b67f7b3efa52a → 41de1e627479b1feb6bd60eec1073bdd1591d490 → c3a2534b8a0c21a9276e5a6fba34f755daaf8e9e`, with expected phase-2 membership `3 affected / 9 unaffected`, hard-cap compatibility `15 reader / 12 evaluator / 0 helper / 0 retry / 27 total`, model `gpt-5.6-sol`, effort `high`, and no Tier 2. The bounded correction adds canonical persisted live-authority resolution, exact four-stage CP9 artifact execution, the real JSONL stdio App Server transport, honest usage measurement and preflight-only CLI. Deterministic verification passes CP9 `11/11`, v2 `132/132`, CP8A `95/95`, CP8B `24/24`, v1 `130/130`, structural `37/37`, repository `11/0/0` and catalog `9/27/187/0`. The single non-model machine preflight resolved the executable but stopped at `APP_SERVER_LAUNCH_FAILED` with `runtime_confirmed_not_started`; protocol readiness and `thread/start`/`turn/start` were not reached. Live calls remain `0`, and CP9 readmission remains `STOP` until launch/readiness is proven.
