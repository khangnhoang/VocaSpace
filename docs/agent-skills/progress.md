...
- Authority: the current task authorizes the bounded CP8B implementation and exactly one coherent local CP8B implementation commit after terminal `0 Critical / 0 Required`. Push, live calls, real App Server cleanup, PR/CI/merge/deploy, history rewrite, CP9 and CP10 remain unauthorized.
