...
**ASM-PR1, ASM-PR2A–ASM-PR2C, ASM-PR3, ASM-PR4, ASM-PR5A và ASM-PR5B đã merge; PR #73 merged ASM-PR5B tại `effb5571955aa09b714e97b7162a6bb3bed0bca4`. Current workstream là Agent Skill Eval Harness hardening trên `refactor/agent-skill-eval-harness`; Stage 1 (`CP1–CP4`) và Stage 2 (`CP5–CP7`) đã completed/delivered, Stage 3 CP8A đã completed, CP8B đã closed với `CP8B_CLOSURE_ACCEPTED`, và CP9 pilot đã dừng fail-closed tại first-reader `thread/start` trước mọi `turn/start` hoặc model/provider call. Runtime-policy decision vẫn là `GLOBAL_ONLY`; exact runtime là global `gpt-5.6-sol / medium`, `config_sha256=4d04305014de339dcafe3902c3446e22e977fcf003250d4156017bd98fd2412a`, helper budget `0` và no escalation role. Bounded fake-only thread-start diagnostic-evidence correction đã implemented; bước kế tiếp là independent re-audit, không phải resume historical run. CP10 chưa bắt đầu.**
...
- Authority: this owner task covers only the bounded CP9 thread-start diagnostic-evidence correction, deterministic fake regressions and terminal self-review. Real App Server/thread/turn/model/provider calls, pilot retry/resume, new preparation/run/authority/probe, commit/push, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
- Stage status: Stage 1 CP1–CP4 is `completed / delivered`; Stage 2 CP5–CP7 is `completed / delivered`; Stage 3 CP8A is `completed` at `f9c821323ae5e8aa277d2cd68d4b416d80cf553e`; CP8B is `CP8B_CLOSURE_ACCEPTED` at `19cd324b7a0fd1509bddf441a2572360013e72f5`; CP9 R1–R4 remain closed, the exact pilot is terminal `blocked`, and the fake-only diagnostic-evidence correction is `implemented / verification passed`, awaiting independent re-audit; CP10 is pending.
...
- CP9 thread-start diagnostic-evidence correction: the exact owner-authorized pilot for preparation `aa09ab71e1bbf33f0056e13738ac86a23114624b190bd2954bc266a1487f274a` and run `cp9-live-aa09ab71e1bbf33f0056e137-run` stopped fail-closed at the first reader thread request with write intent `1`, acknowledgement `0`, outcome unknown `1`, `turn/start=0` and model/provider calls `0`. The historical journal remains immutable and insufficient to prove whether request bytes left the harness or whether the App Server created a shadow thread. Production transport/adapter/journal now persist bounded future-only markers for completed write, observed response and sanitized failure diagnostics: error category/class/code, process exit code/signal/timing, response-observed/classification, stable RPC error code, and stderr byte count/SHA-256 only. No raw stderr, response bytes, exception message, credential or secret is persisted. Legacy journals remain valid; the terminal blocked run cannot resume and is not retroactively enriched. Deterministic evidence passes focused CP9 thread-start `8/8`, focused CP8A `3/3`, full CP8A `96/96`, shared v2 `133/133`, v1 compatibility `130/130`, structural `37/37`, repository `11/0/0`, catalog `9/27/187/0/0`, syntax, targeted ESLint and diff checks. The broader CP9 run was intentionally stopped before its long topology fixture after upstream cases passed; no real App Server/thread/turn/model/provider call, new runtime artifact, authority, preparation or run was created by this correction. Next gate is independent diagnostic re-audit.
