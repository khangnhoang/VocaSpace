...
**Harness follow-up current status ngày 2026-09-07:** owner-approved scope gồm explicit cross-run reader donor reuse và configurable reader model/effort; default reader/evaluator giữ `gpt-5.6-sol / medium`. Authorized fetch xác nhận clean `main == origin/main == 2be02df11e279b5c88f37d2fd609069a54c235ed`, ahead/behind `0/0`; branch `codex/harness-follow-up-plan` tạo từ đó, discovery worktree cũ giữ nguyên. [Detailed plan](./implementation-plans/eval-harness-follow-up/plan.md) và [owner brief](./implementation-plans/eval-harness-follow-up/owner-review-brief.md) là `frozen implementation contract`: CP1 donor lifecycle, CP2 reader configuration, mỗi checkpoint tách source/tests/docs và verification; CP3 đã prepare exact canary manifest sau implementation nhưng không chạy live/model. Owner thay đề xuất 6-case calibration bằng 3–4 cases, tối đa `18` hoặc `24` dispatch, retry `0`, evaluator Sol Medium; canary đạt chỉ đề xuất mở rộng, không tự fanout/default change. CP0 frozen-plan commit là `d093b08`; initial CP1 evidence `9/9` targeted và `105/105` full CLI sau đó được xác định thiếu composed integrity-matrix coverage; CP1 correction pass `35/35` targeted và `131/131` full CLI, đã được ghi thành correction commit riêng không amend `da72752`; CP2 deterministic implementation/tests/docs đã pass với targeted `6/6`, full CLI `137/137`, cumulative v1 `130/130` và validators `valid`; CP3 deterministic prepare đã tạo exact two-arm manifest với dispatch `0`. Live/model/push/PR/merge chưa thực hiện; `0 specialist`.
...
- `report` exit `1` với `incomplete` là expected vì chưa chạy closure nào; không phải canary failure. Live authority, billing/auth observation, model/provider/evaluator/egress/push/PR/merge vẫn chưa cấp hoặc chưa quan sát. Chưa stage/commit; suggested message: `docs(skill-evals): finalize Luna canary grant manifest`.
