...
**ASM-PR1, ASM-PR2A–ASM-PR2C, ASM-PR3, ASM-PR4, ASM-PR5A và ASM-PR5B đã merge; PR #73 merged ASM-PR5B tại `effb5571955aa09b714e97b7162a6bb3bed0bca4`. Current workstream là Agent Skill Eval Harness hardening trên `refactor/agent-skill-eval-harness`; Stage 1 (`CP1–CP4`) đã được owner authorize implementation, còn Stage 2+, live model/helper/evaluator/provider calls và PR/merge/deploy chưa được authorize.**
...
- Stage 1 status: CP1–CP4 `implemented / deterministic checks passed / review passed`; cumulative Stage 1 review passed at `0 Critical / 0 Required`; delivery verification and the one authorized normal push remain. No CP5+ work has started.
