...
**ASM-PR1, ASM-PR2A–ASM-PR2C, ASM-PR3, ASM-PR4, ASM-PR5A và ASM-PR5B đã merge; PR #73 merged ASM-PR5B tại `effb5571955aa09b714e97b7162a6bb3bed0bca4`. Current workstream là Agent Skill Eval Harness hardening trên `refactor/agent-skill-eval-harness`; Stage 1 (`CP1–CP4`) và Stage 2 (`CP5–CP7`) đã completed/delivered, Stage 3 CP8A đã completed tại `f9c821323ae5e8aa277d2cd68d4b416d80cf553e`, và CP8B đã closed tại `19cd324b7a0fd1509bddf441a2572360013e72f5` với `CP8B_CLOSURE_ACCEPTED`, `0 Critical / 0 Required / 0 Advisory`. CP9 bounded live transport/launch correction đã implemented và deterministically verified; standalone non-model preflight đã đóng launch blocker và readmit existing Tier 1 workload. Real pilot vẫn `not_run`, chưa có live authorization, và model/helper/evaluator/provider/subagent calls vẫn bằng `0`. CP10 chưa bắt đầu.**
...
- Authority: the current task authorizes CP9 readmission documentation and exactly one local docs commit. Real pilot/model/provider/reader/evaluator/helper/subagent calls, push, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
- Stage status: Stage 1 CP1–CP4 is `completed / delivered`; final delivered Stage 1 HEAD is `54453746b2ea796558ef229831a569a69c4ed3f4` and terminal review is `0 Critical / 0 Required / 1 Advisory`. Stage 2 CP5–CP7 is `completed / delivered` with terminal cumulative review `0 Critical / 0 Required`. Stage 3 CP8A is `completed` at `f9c821323ae5e8aa277d2cd68d4b416d80cf553e`; CP8B is `CP8B_CLOSURE_ACCEPTED` at `19cd324b7a0fd1509bddf441a2572360013e72f5`; CP9 Tier 1 is readmitted for a separate explicit live-authorization decision, but the real pilot is still `not_run`; CP10 is pending.
...
- CP9 admission/readmission evidence: historical generations remain separate and are not a live A/B. ASM-PR3 full-fanout reader usage is `61` (`reconstructed` from `18 + 1`, `36 + 2`, and `4` durable observations); semantic-substitution dogfood is `62 reader / 0 helper / 0 evaluator` (`exact_observed`). Historical model/runtime/tokens/wall-clock and complete model-visible input/output bytes are `unavailable`. Tier 1 remains the staged ASM-PR5B range `3fa621c86399e5c1a9e43bd9cd7b67f7b3efa52a → 41de1e627479b1feb6bd60eec1073bdd1591d490 → c3a2534b8a0c21a9276e5a6fba34f755daaf8e9e`, with expected phase-2 membership `3 affected / 9 unaffected`, final maximum `15 reader / 12 evaluator / 0 helper / 0 automatic retry / 27 total`, model `gpt-5.6-sol`, effort `high`, mandatory canary-before-fanout/stop-on-failure, and no Tier 2. Exact standalone identity is `C:\Users\khang\.codex\packages\standalone\releases\0.149.1-x86_64-pc-windows-msvc\bin\codex.exe`, `codex-cli 0.149.1`, SHA-256 `a395030b56b126f608f2403036dddb654a9c063213e9c2b5f85d954cf490ebe6`. One non-model preflight returned `executable_resolution=resolved`, `process_launch=started`, `protocol_readiness=ready`, account type `chatgpt`, model `gpt-5.6-sol`, effort `high`, `thread_creation=not_started`, `turn_dispatch=not_started`, `model_calls_dispatched=0`. Exact token usage remains `unavailable` unless later live App Server responses expose trustworthy retained usage. Bounded readmission review is `0 Critical / 0 Required`; real pilot execution still requires separate explicit live authority and remains `not_run`.
