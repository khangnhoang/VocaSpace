...
- Planned order: `CP1 compatibility/v2 contracts → CP2 logical identities/impact → CP3 durable state/attempts/resume → CP4 reader/evaluator-static readiness and bounded helpers → CP5 sequential reader/reuse → CP6 evaluator stage guard/advisory proposal/human acceptance/report → CP7 controls/concurrency → CP8A concrete provider adapter certification → CP8B retention/legacy/CI/docs → separately-authorized CP9 six-case pilot → CP10 cumulative review/delivery decision`.
...
- Current proportional verification: eval catalog `9 skills / 27 files / 183 cases / 0 diagnostics`; repository validator `11 skills / 0 errors / 0 warnings`; CLI no-model claim, four-file links/UTF-8/no-BOM/final-newline/conflict/machine-path/actionable-TODO, exact six case IDs, A–G contract assertions and `git diff --check` pass. Parent source/test evidence remains current because no code/skill/test changed: eval-runner `130/130`, structural-validator `37/37`; those suites were not needlessly rerun.
