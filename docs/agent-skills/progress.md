...
- CP9 thread-start diagnostic-evidence R2 correction: the exact owner-authorized pilot for preparation `aa09ab71e1bbf33f0056e13738ac86a23114624b190bd2954bc266a1487f274a` and run `cp9-live-aa09ab71e1bbf33f0056e137-run` remains terminal `blocked` with historical write intent `1`, acknowledgement `0`, outcome unknown `1`, `turn/start=0` and model/provider calls `0`. Two independent audits found two Required semantics in the first future-only diagnostic correction. R2 now records raw stdout only as `response_channel_bytes_observed`; `thread_start_response_observed` and `response_bytes_observed` require a parsed exact pending request ID, so a valid unrelated notification remains `unrelated_notification_observed` without a false matching-response event. JSON syntax failure remains `json_invalid`, while syntactically valid credential/protocol rejection is `protocol_invalid`. Adapter fallback now persists unavailable stderr as `null/null`; only transport-observed stderr uses byte count plus SHA-256, including valid observed-empty `0 + SHA256(empty)`. Legacy journals remain valid; the terminal historical run is unchanged, non-resumable and still has insufficient root-cause evidence. Deterministic evidence passes focused CP9 thread-start `10/10`, focused CP8A `4/4`, full CP8A `97/97`, shared v2 `133/133`, v1 compatibility `130/130`, structural `37/37`, repository `11/0/0`, catalog `9/27/187/0/0`, syntax, targeted ESLint and diff checks. Long CP9 topology did not run because the correction does not touch topology. No real App Server/thread/turn/model/provider call, runtime artifact, authority, preparation or run was created. Next gate is independent R2 diagnostic re-audit.
