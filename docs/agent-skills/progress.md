...
- Authority: current owner authority covers the bounded five-finding CP9 whole-PR pre-pilot correction, exactly one coherent commit and one normal push after terminal `0 Critical / 0 Required`. Pilot execution, provider/model/reader/evaluator/helper/subagent calls, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
