...
- CP8A evidence: `codex_chatgpt_app_server` now implements the frozen `runtime_mediated`/`chatgpt_subscription` boundary through an injected JSONL App Server transport contract. Immutable `run_manifest.intent`, exact prepared-attempt/readiness/grant/runtime lineage, ChatGPT-only auth, output-schema/effective-policy/config/instruction-source attestation, explicit unsupported-lookup limitation, exact secret-safe `thread/start` journal evidence, atomic pre-`turn/start` `input.txt`/`request.json` snapshots, append-only runtime events, finder indexes, result/evidence binding, live-limit guards, exact same-run reuse/restart validation and cross-run opaque-envelope `unknown` reuse all fail closed. Reader/evaluator Stage 2 materialization and verification-helper serialization are covered; connect/dispatch/response timeout, interrupt, lookup, shadow-thread recovery, stale grant/runtime/representation and one-dimension semantic-substitution matrices use deterministic fake transport only. Focused CP8A `38/38`, full v2 `132/132`, v1 compatibility `130/130`, structural `37/37`, repository `11/0/0`, catalog `9/27/187/0`, syntax/diff/CLI/hygiene checks pass; terminal review is `0 Critical / 0 Required`. Live model/helper/evaluator/provider/subagent execution calls: `0`.
- Remaining Advisories: full Node-on-POSIX remains `not_run` because local WSL2 lacks Node; the earlier semantic-dogfood synthetic package sometimes caused reader over-selection of `mocking-and-regression.md`; and that historical manual adapter remained instruction/self-report-bound. CP8A deterministic certification does not itself authorize or claim a live CP9 App Server/model run.
