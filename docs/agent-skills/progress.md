...
- Branch/base: Stage 2 `feat/agent-skill-eval-cli-prepare` merged qua PR #79 tại `d4b5f78a09fa997da75e969822bbc678c7f002a4`; current Stage 3 branch `feat/agent-skill-eval-cli-reuse` được tạo từ exact merge đó.
- Status: Stage 0 `completed / merged`; Stage 1 `completed / actual CLI gate passed / merged`; Stage 2 `completed / merged`; Stage 3 `completed locally / 0 Critical / 0 Required / not pushed`. Stage 3 final deterministic evidence: focused Windows `71/71`, v1 compatibility `130/130`, structural validator `37/37`, repository validator `11 skills / 0 errors / 0 warnings`, eval catalog `9 skills / 27 files / 187 cases / 0 diagnostics`, syntax và `git diff --check` pass.
- Current authority: owner prompt ngày `2026-09-01` authorize implement frozen Stage 3 contract và follow-up commit policy authorize local checkpoint commits sau deterministic verification + formal `0 Critical / 0 Required`. Local commits `7663c72`, `0536d43`, `7a03fa4`, `745c61f`, `d81dd54` đã consume các coherent CP0–CP4 boundaries; CP5 cumulative correction/progress commit thuộc cùng explicit policy. Không authorize push, PR, CI-fix, merge hoặc bất kỳ remote/live model/evaluator action nào.
...
- Detailed plans: [CLI-first master plan](./implementation-plans/eval-harness-cli-first/plan.md), [Stage 1 CLI runner plan](./implementation-plans/eval-harness-cli-first/stage-1-cli-runner.md), historical [Stage 2 prepare plan](./implementation-plans/eval-harness-cli-first/stage-2-cli-prepare.md), current [Stage 3 reuse plan](./implementation-plans/eval-harness-cli-first/stage-3-cli-reuse.md); decision surface: [owner review brief](./implementation-plans/eval-harness-cli-first/owner-review-brief.md).
...
- Latest Stage 3 transferable-contract correction: all four supplied findings were independently confirmed `Required`. Full `active_attempt` identity/ordinal/producer revision/paths now persist before spawn; recovery preserves valid terminal results, creates recovery-only `outcome_unknown` only when no result/record exists, and integrity-blocks contradictory evidence without fake relationships or extra ordinals. Unit fingerprint/binding nullability, lexical exact dependency-binding equality and persisted-integrity versus derived-dependency blocking are frozen. Public commands now own canonical result/error envelopes, aggregate run-status/reason derivation and exact exit `0/1/2/3`. Completion wording explicitly allows Stage 3-owned `cli_run`/`cli_execution_plan` v2 while still forbidding import/edit of existing App Server/CP9/v2 modules. Docs verification passes: repository skill validator `11 skills / 0 errors / 0 warnings`, eval catalog `9 skills / 27 files / 187 cases / 0 diagnostics`, `git diff --check`, stale-contract scan and exact no-diff audit of historical Stage 2 plan. Source/test/model/evaluator/live calls remain `0`; implementation/commit/remote authority remains absent.
- Continuation Stage 3 contract correction: all four follow-up findings were independently confirmed `Required`. Master/detail now distinguish fully valid old evidence with changed current behavior/bindings (`pending` invalidated) from corrupt/missing/substituted accepted evidence (no-mutation `command_error`); persisted integrity block is restricted to coordinator-owned in-flight recovery contradictions. `operational_condition` is a persisted preflight latch above ready-work derivation with read-only reporting and passing-preflight clear lifecycle; `prepare --run` preserves that latch because it performs no execution preflight. Canonical v1 `status` derives its full zero-attempt snapshot in memory without store writes. Patch-check publishes mixed mode as a standalone first `run.json` mutation, survives all same-revision commands and resets only on next successful revision publication. Regression plan also requires all-or-nothing prevalidation for multi-unit retry/patch-check. Current docs verification passes: repository skill validator `11 skills / 0 errors / 0 warnings`, eval catalog `9 skills / 27 files / 187 cases / 0 diagnostics`, `git diff --check`, link/fence/UTF-8/newline/whitespace audit, source/CI scope audit and exact no-diff audit of historical Stage 2 plan. Source/test/model/evaluator/live calls remain `0`; implementation/commit/remote authority remains absent.
- Final Stage 3 transferability correction: both new findings were independently confirmed `Required`. Frozen `max_attempts` is now a run-wide lifetime ceiling per logical unit across revisions/fingerprints; every worker-backed/recovery ordinal and every spawn path uses the same gate. A later revision may truthfully publish an exhausted invalidated reader as persisted pending + derived `blocked / attempt_budget_exhausted`, but settled execution exits `1`, no path dispatches it, and only a new run resets budget. Patch-check has an exact projected reader/evaluator status/dependency/budget matrix and remains all-or-nothing. Recognized `operational_condition` is strictly pre-spawn, so canonical error dispatch is always `0`; post-spawn failures remain attempt-local terminal outcomes and no mid-dispatch latch subsystem was added. Docs verification passes: repository skill validator `11 skills / 0 errors / 0 warnings`, eval catalog `9 skills / 27 files / 187 cases / 0 diagnostics`, `git diff --check`, stale-contract scan and exact no-diff audit of historical Stage 2 plan. Source/test/model/evaluator/live calls remain `0`; implementation/commit/remote authority remains absent.
- Stage 3 implementation evidence: CP0 binds every accepted result to immutable attempt/result/output plus producing revision descriptor; CP1 adds strict v2 run/unit/attempt state, v1 bootstrap and revision recovery; CP2 adds canonical `run`/read-only `status`, reader attempts, evaluator preparation and all restart windows; CP3 adds `resume`, atomic explicit `retry`, pure startup projection and the persisted operational latch; CP4 adds impact closure and `patch-check_mixed_revision`; CP5 cumulative review corrected next-revision invalidation/public prepare snapshots so changed accepted success becomes persisted pending before `run.json`-last publication and corrupt accepted evidence creates no next prepared revision. Final local gates pass at focused `71/71`, v1 `130/130`, structural `37/37`, repository `11/0/0`, catalog `9/27/187/0`, syntax/diff/scope/conflict/secret scans; cumulative formal review is `0 Critical / 0 Required`. Deterministic fake/local temp adapters only; evaluator dispatch, installed Codex invocation, network, model/provider/live calls all remain `0`.
