...
**ASM-PR1, ASM-PR2A–ASM-PR2C, ASM-PR3, ASM-PR4, ASM-PR5A và ASM-PR5B đã merge; PR #73 merged ASM-PR5B tại `effb5571955aa09b714e97b7162a6bb3bed0bca4`. Current workstream là Agent Skill Eval Harness hardening trên `refactor/agent-skill-eval-harness`; Stage 1 (`CP1–CP4`) và Stage 2 (`CP5–CP7`) đã completed/delivered, còn Stage 3 CP8A đã completed tại terminal review `0 Critical / 0 Required`. CP8A certification chỉ dùng deterministic mocked App Server transport; live model/helper/evaluator/provider calls bằng `0`. CP8B+ vẫn pending explicit owner authorization và exact latest delivery state thuộc Git evidence.**
...
- Authority: no standing Stage 1–2 authority remains. The bounded CP8A implementation/local-checkpoint authority is consumed by its completed checkpoint; no push, CP8B+, live-call, PR/CI/merge/deploy or history-rewrite authority carries forward.
- Stage status: Stage 1 CP1–CP4 is `completed / delivered`; final delivered Stage 1 HEAD is `54453746b2ea796558ef229831a569a69c4ed3f4` and terminal review is `0 Critical / 0 Required / 1 Advisory`. Stage 2 CP5–CP7 is `completed / delivered` with terminal cumulative review `0 Critical / 0 Required`. Stage 3 CP8A is `completed` at terminal review `0 Critical / 0 Required`; CP8B–CP10 remain pending their exact gates. Exact latest delivery state belongs to Git evidence.
...
- CP8A evidence: `codex_chatgpt_app_server` implements the frozen `runtime_mediated`/`chatgpt_subscription` boundary through an injected JSONL App Server transport contract. The bounded correction after `6917114` adds one allowlisted immutable behavior-runtime projection to reader/evaluator identity; retains exact secret-safe inbound JSONL bytes plus body↔outer request/thread/turn lineage; adds durable `verification_helper_input` ownership for cluster/index/question/context/invocation/runtime/run/readiness reconstruction; and passes explicit owner-issued live authority unchanged through canonical reader/evaluator/concrete-helper APIs while absence, intent-only, or substitution still fails before dispatch. `runtime/index.json` and `runtime/index.md` remain independently atomically replaced, rebuildable, non-authoritative views: the final pre-wire gate independently derives both expected current views and exact-compares both, without pair-generation/pointer/transaction machinery. Complete source/donor semantic-substitution regressions cover runtime/config/instruction identity, inner event lineage, helper owner, live grant, finder generations and valid differently-bound `input.txt`/`request.json`, with owner-specific failures, zero new `turn/start` writes and zero semantic descendants. Focused CP8A `75/75`, full v2 `132/132`, v1 compatibility `130/130`, structural `37/37`, repository `11/0/0`, catalog `9/27/187/0`, plus CLI/syntax/diff/UTF-8/no-BOM/final-newline/hygiene checks pass; deterministic fake/mock transport only. Live model/helper/evaluator/provider/subagent execution calls: `0`.
- Terminal bounded-correction review: `0 Critical / 0 Required / 4 Advisory`. Remaining Advisories: full Node-on-POSIX remains `not_run` because local WSL2 lacks Node; the earlier semantic-dogfood synthetic package sometimes caused reader over-selection of `mocking-and-regression.md`; that historical manual adapter remained instruction/self-report-bound; and an orphaned live-budget reservation lock after hard process crash remains an accepted fail-closed liveness limitation. CP8A deterministic certification does not itself authorize or claim a live CP9 App Server/model run.
