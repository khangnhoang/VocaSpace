...
Verification workflow correction: focused fake-worker resume/failure-isolation/downstream-retry/exhausted-budget tests `5/5`, `git diff --check` pass. Plan ghi đủ pause/end-of-pass evidence, exact resume path và phương án follow-up selected closure cho unit hết budget; không hứa cross-run success import hoặc single-run report completion. Semantic acceptance vẫn cần đủ 22 coherent current case graphs cho final candidate và reviewer/owner decisions, không suy từ việc đã chạy hết runnable work.
