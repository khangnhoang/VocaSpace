...
**ASM-PR1, ASM-PR2A–ASM-PR2C, ASM-PR3, ASM-PR4, ASM-PR5A và ASM-PR5B đã merge; PR #73 merged ASM-PR5B tại `effb5571955aa09b714e97b7162a6bb3bed0bca4`. Current workstream là Agent Skill Eval Harness hardening trên `refactor/agent-skill-eval-harness`; Stage 1 (`CP1–CP4`) và Stage 2 (`CP5–CP7`) đã completed/delivered, Stage 3 CP8A đã completed tại `f9c821323ae5e8aa277d2cd68d4b416d80cf553e`, và CP8B đã closed tại `19cd324b7a0fd1509bddf441a2572360013e72f5` với `CP8B_CLOSURE_ACCEPTED`, `0 Critical / 0 Required / 0 Advisory`. CP9 admission/measurement design đã complete nhưng live admission là `STOP`; live model/helper/evaluator/provider/subagent calls vẫn bằng `0`. CP10 chưa bắt đầu.**
...
- Authority: the current task authorizes CP9 admission and measurement design only. Live calls, external App Server mutation, implementation, commit, push, PR/CI/merge/deploy, history rewrite and CP10 remain unauthorized.
- Stage status: Stage 1 CP1–CP4 is `completed / delivered`; final delivered Stage 1 HEAD is `54453746b2ea796558ef229831a569a69c4ed3f4` and terminal review is `0 Critical / 0 Required / 1 Advisory`. Stage 2 CP5–CP7 is `completed / delivered` with terminal cumulative review `0 Critical / 0 Required`. Stage 3 CP8A is `completed` at `f9c821323ae5e8aa277d2cd68d4b416d80cf553e`; CP8B is `CP8B_CLOSURE_ACCEPTED` at `19cd324b7a0fd1509bddf441a2572360013e72f5`; CP9 live admission is `STOP`; CP10 is pending.
...
- CP9 admission/measurement design: historical generations remain separate. ASM-PR3 full-fanout reader usage is `61` (`reconstructed` from `18 + 1`, `36 + 2`, and `4` durable observations); semantic-substitution dogfood is `62 reader / 0 helper / 0 evaluator` (`exact_observed`). Historical model/runtime/tokens/wall-clock and complete model-visible input/output bytes are `unavailable`. Tier 1 selects the real staged ASM-PR5B range `3fa621c86399e5c1a9e43bd9cd7b67f7b3efa52a → 41de1e627479b1feb6bd60eec1073bdd1591d490 → c3a2534b8a0c21a9276e5a6fba34f755daaf8e9e`, with expected phase-2 membership `3 affected / 9 unaffected`, proposed hard cap `15 reader / 12 evaluator / 0 helper / 0 retry / 27 total`, model `gpt-5.6-sol`, effort `high`, and no Tier 2. The 12 evaluator limit follows the current one-evaluator-per-reader-unit contract. Admission is `STOP` because the current CLI has no model-execution command, repository transport implementations are mock-only, and direct local Codex/App Server executable inspection fails `Access is denied`; a bounded CP9-only live transport/launch correction and repeated admission are required. Live calls remain `0`.
