...
- Implementation permission: AW-PR2 behavior implementation đã hoàn tất; instruction hiện tại chỉ cấp final cumulative audit, in-scope correction và PR/CI delivery cho exact branch. Không cấp AW-PR3A/AW-PR3B hoặc behavior implementation mới.
...
- Permission boundary hiện tại: final cumulative audit, smallest in-scope correction, local checkpoint, normal push, create/update PR against `main`, CI watching và exact bounded `branch-caused-small-safe` loop được cấp. Merge, auto-merge, force-push, branch deletion, specialist/sub-agent, AW-PR3A/AW-PR3B, production/deployment/database/remote-environment và unrelated scope không được cấp.
