...
- Permission boundary hiện tại: được implement CP2 và tạo coherent local checkpoint sau khi 0 Critical/Required; không push, PR create/update, CI watching, merge, force-push, specialist/sub-agent, production/destructive action hoặc CP3 implementation.
...
| CP2 — CI permission reconciliation | `verified; ready for local checkpoint` | `docs/agent-loops.md`, `.agents/skills/git-checkpoint-workflow/SKILL.md`, `.agents/skills/github-pr-ci-workflow/SKILL.md`; supporting tracker/decision records only from actual evidence | Validator, permission scenarios, docs/text/link/table/scope/secret/Git audits pass; formal re-review has 0 Critical, 0 Required; fresh-reader `not_run` | pending local checkpoint; no push permission |
