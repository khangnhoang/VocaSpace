...
Các non-goals vẫn giữ nguyên: không cross-chapter topic movement, không bulk reorder, không restore/purge UI, không cascade archive/restore, không analytics và không mascot/illustration. Production DB migration/preflight cho PR7 ordering đã được xử lý trong session trước; hardening pass hiện tại không tạo DB deployment gate mới.

### 2026-07-04: PR10 mobile QA và release-gate clarification

PR10 mobile QA hardening đã được implement và push ở commit `f22367b fix(course-workspace): harden PR10 mobile QA issues`. Desktop vẫn là primary authoring target cho MVP. Mobile flashcard/exercise add/edit controls phức tạp intentionally không expose trong MVP; đây là accepted limitation/release note, không phải MVP blocker.

PR10 release hardening hiện tại chỉ chạm docs, course/card action boundaries, schemas, và focused tests; không thêm DB migration, RLS change, hoặc RPC change. `production-gate` đã tồn tại/enabled, và PR7 ordering migration/RPC đã được apply lên production trong session trước, nên không còn pending owner approval trước DB apply cho pass này.
